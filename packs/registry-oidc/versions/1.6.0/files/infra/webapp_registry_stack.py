...
- One ECR repository per project (e.g. ``example-core``). Image artifacts are
...
The delivery role has an inline action/resource policy for release operations.
Infrastructure changes remain operator-attended through local ``aws-admin``
authority, so repository code cannot rewrite its own custody boundary.
...
class WebappRegistryArgs:
    ...
    # Full repository name (e.g. ``"example-core"``). The caller composes it;
    # no Pack-level default so the project configuration stays explicit.
    repository_name: str
    ...
    # Opt-in delivery grant; ``None`` leaves the policy exactly as it was
    # before this existed. See ``webapp_registry_delivery_ssm_policy``.
    delivery_authority: object | None = None
...
def _github_trust_policy_json(
    ...
    *,
    ...
    subjects = [
        f"repo:{github_repo}:ref:refs/heads/{branch}" for branch in allowed_branches
    ]
    subjects.extend(
...
class WebappRegistryStack(pulumi.ComponentResource):
    ...
    def __init__(
        ...
        if args.github_repo:
            self._create_github_delivery_role(args, tags, child_opts)
    ...
    def _create_github_delivery_role(
        self,
        args: WebappRegistryArgs,
        tags: dict,
        child_opts: pulumi.ResourceOptions,
    ) -> None:
        """Provision the OIDC provider (or reference it) and delivery role."""
        if not args.state_bucket or not args.kms_key_alias:
            raise ValueError(
                "GitHub delivery role requires state_bucket and kms_key_alias"
            )
        if args.manage_github_oidc_provider:
            ...
            provider_arn = (
                f"arn:aws:iam::{args.aws_account_id}:oidc-provider/{_GITHUB_OIDC_HOST}"
            )
        ...
        from webapp_registry_ci_policy import delivery_policy_json
        ...
        aws.iam.RolePolicy(
            ...
            policy=delivery_policy_json(
                ...
                delivery_authority=args.delivery_authority,
            ),
            opts=child_opts,
        )
        pulumi.export("githubActionsDeliveryRoleArn", self.delivery_role.arn)
        from webapp_registry_github_variables import create_delivery_role_variable

        self.delivery_role_variable = create_delivery_role_variable(
