...
from webapp_vps_power_schedule import (
    VpsPowerScheduleArgs,
    attach_power_schedule,
)

# Graviton (ARM64) instance families supported by this Pack.
...
class WebappVpsArgs:
    ...
    # ``_vps_args_from_config`` — no Pack-level default.
    stack_name: str
    ...
    # When declared, the instance is stopped and started on a cron instead of
    # billing around the clock. Absent (the default) leaves it always on.
    power_schedule: VpsPowerScheduleArgs = field(
        default_factory=VpsPowerScheduleArgs
    )
