...
## The four operations a person runs against a machine

Every machine declares its `host_kind` in settings, and each kind implements
the same four operations. They are commands, not procedures to follow by hand:

```text
yoke test-machine verify --project P --machine NAME
yoke test-machine reset --project P --machine NAME [--baseline fresh-host|shell-preconfigured]
yoke test-machine golden-capture --project P --machine NAME [--destination /abs/path] [--probes-file FILE]
yoke test-machine bridge-diagnose --project P --machine NAME
```

- **verify** is the readiness gate. It reaches BOTH registered baselines in
  order, so the box it hands back carries the current Yoke launcher and is
  **not** fresh. Its receipt says so in words.
- **reset** reaches exactly ONE baseline and stops. `fresh-host` is the
  default, and it is the command that hands an operator a clean machine.
- **golden capture** copies the host's home into a new dated baseline
  directory, seals its manifest and probes sidecars, and records the new path
  on the machine — but only after every declared probe passes and no Yoke
  residue is present. It refuses by naming the exact residue path, the exact
  failing probe, or the occupied destination.
- **bridge diagnose** exercises each Terminal-bridge capability on its own and
  names, per failing capability, the host condition to change: the missing
  privacy grant, a console owned by another login, a locked screen, Secure
  Keyboard Entry, or a window that never became frontmost. Reach for it when
  verification reports a bridge failure.

Each runs under the machine's one lease and refuses by name while another
execution holds it. Each records its own receipt, and the machine's page shows
the last one per operation, so what was last done to a box is readable without
