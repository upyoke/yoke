...
All three select Yoke's approved `host_control` executor and require one
...
Method definitions are installed at `qa/methods/machine-qa.json`. Installed
files belong to the project and may be customized there; the Pack receipt is
only the update baseline.
