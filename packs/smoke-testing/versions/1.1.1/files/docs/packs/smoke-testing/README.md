...
flow's smoke stage must declare it (`yoke deployment-flows create` at
definition time, `yoke deployment-flows update-stages` afterwards):
...
`commit_sha` is a required `workflow_dispatch` input. It must be one full
40-hex-character source commit SHA. The smoke stage should pass the exact
commit the deployment run shipped, typically via `{head_sha}`:
...
The workflow validates that SHA, passes it into the comparison step through an
environment variable, and always compares it to the host's `.deploy-version`
marker. A missing marker or a different SHA fails the check.
