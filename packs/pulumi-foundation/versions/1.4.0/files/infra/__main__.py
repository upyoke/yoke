...
def _vps_args_from_config(deploy_namespace: str):
    ...
    return WebappVpsArgs(
        ...
        iam_instance_profile_name=(config.get("vps_iam_instance_profile_name") or None),
...
def _registry_args_from_config(deploy_namespace: str):
    from webapp_registry_delivery_ssm_policy import delivery_authority_from_config
    ...
    return WebappRegistryArgs(
        ...
        # Opt-in; absent leaves the delivery role's policy unchanged. The
        # registry Pack owns the descriptor and validates it, because it owns
        # the role the grant bounds.
        delivery_authority=delivery_authority_from_config(
            config.get_object("delivery_authority")
        ),
...
def _environment_args_from_config(deploy_namespace: str):
    ...
    return WebappEnvironmentArgs(
        ...
        origin_vps_elastic_ip_output=config.require("origin_vps_elastic_ip_output"),
