...
FLEET_STEERING_ADDRESSING_GUIDANCE = (
    ...
    "reaches the seat once, so a reworded retry of the same completion is "
    "deduplicated rather than delivered twice. A completion you were "
    "RESUMED to do is its own leg and is delivered, whether that resume "
    "handed you a fresh claim or left your lane held — never release an "
    "unfinished lane merely to be heard. A send answering `Collapsed into "
    "an earlier message` discarded the body you just sent: read it rather "
    "than assume you reported, and if it really was a second completion, "
