...
FLEET_STEERING_ADDRESSING_GUIDANCE = (
    ...
    "the turn you send it in, under the claim you send it under — reaches "
    "the seat once, so a reworded retry in that same turn is deduplicated "
    "rather than delivered twice. A completion you were RESUMED to do is "
    "reported from a later turn, so it is delivered rather than collapsed "
    "into the first, whether the resume handed you a fresh claim or left "
    "your lane held — never release an unfinished lane merely to be heard. "
    "A send answering `Collapsed into an earlier message` discarded the "
    "body you just sent; read it rather than assume you reported. "
    "Ending a turn sends "
