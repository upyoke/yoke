...
FLEET_STEERING_ADDRESSING_GUIDANCE = (
    ...
    "correct across a seat handoff, which a session id cannot. A DONE "
    "PREFIX-N heading is the report identity: it must name work this "
    "session holds or released, and that named item addresses the seat so a "
    "live claim on another item cannot steal the key. Send the DONE before "
    "releasing a claim you still hold; one terminal report per work leg — "
    "your claim on the named item plus the instruction you last "
    "acknowledged — reaches the seat once, so a reworded retry is "
    "deduplicated rather than delivered twice. A resume opens a NEW leg "
    "whether it hands you a fresh claim or leaves your lane held, because "
    "you acknowledge the instruction authorizing it, so that second "
    "completion is delivered rather than collapsed into the first — never "
    "release an unfinished lane merely to be heard. A send answering "
    "`Collapsed into an earlier message` discarded the body you just sent; "
    "read it rather than assume you reported. Ending a turn sends "
