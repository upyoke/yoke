...
FLEET_STEERING_ADDRESSING_GUIDANCE = (
    ...
    "your claim on the named item — reaches the seat once, so a reworded "
    "retry is deduplicated rather than delivered twice. Newly authorized "
    "work on that same item is a NEW leg: reacquire the item claim before "
    "reporting it, and that second completion is delivered rather than "
    "collapsed into the first. A send answering `Collapsed into an earlier "
    "message` discarded the body you just sent; read it rather than assume "
    "you reported. Ending a turn sends "
