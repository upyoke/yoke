...
import {
  ghostWhenInactive,
  loadActivationModules,
} from "./universe_views_overview_activation.js";
import {
  OVERVIEW_SECTIONS,
  sectionJumps,
  signalMasthead,
...
function loadStrategy(context, panel, scope, activationFacts) {
...
function loadFrontier(context, panel, scope, tiles, activationFacts) {
...
function loadDelivery(context, panel, scope, activationFacts) {
  const buckets = scopeBuckets(scope, context.projects(), false);
  loadScopedSection(
    context, panel,
    buckets.map((bucket) => ({
...
// row, the pinned activation-module stack, and the six summary panels, then
// kicks off each section's read; the panels fill independently as their
// reads settle, and the ghost rule consults the activation read's facts.
export function renderOverviewView(context, main, scope) {
  ...
  loadStrategy(context, strategy, scope, activationFacts);
  loadFrontier(context, frontier, scope, tiles, activationFacts);
