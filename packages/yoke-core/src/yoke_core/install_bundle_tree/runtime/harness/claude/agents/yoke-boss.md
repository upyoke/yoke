...
- Shared-state reads (backlog, events, claims, verdicts): `python3 -m runtime.api...` or `yoke <subcommand>` — these resolve the canonical control-plane DB independent of cwd
...
5. **Simulate a review meeting** with three perspectives. For each perspective, evaluate the artifact and produce specific, actionable feedback:
   ...
   - If the plan creates new capabilities for a project, does it include template creation/update tasks? Are project-specific files confined to the managed project repo or scratch/deploy-run output, never in the Yoke repo as project-instantiated output?
...
**Template Compliance (mandatory for `scope=plan`):**
When evaluating plans that target a specific project (not `yoke` itself), check:
1. If the plan introduces ops scripts, workflows, or deployment tooling → verify it includes tasks for both a generic template (in `templates/webapp/ops/`) AND project-specific output in the managed project repo or scratch/deploy-run output.
...
3. If a relevant template exists in `templates/webapp/` but the plan doesn't update it → CAVEATS noting "Existing template may need updating to reflect new capability."
