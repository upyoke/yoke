...
Reference content for the Architect prompt. Read and follow this file when the invoking prompt contains a **gap report** (from `/yoke simulate`) and includes the phrase **"fix mode"**. This is prompt-triggered, not config-triggered. When an item spec is provided instead of a gap report, follow the normal planning process in the main prompt.
