...
# The wrapper prints wait_mode and why. background-wake emits the bound pair; in-turn runs here until exit; after a background-wake completion, tail -80 <raw-capture>.
...
tail -80 <PATH>`
  - --print-streaming-pair chooses the safe wait mode and mints the capture path automatically via project_scratch_dir.mint_watcher_capture_pair (machine temp root watcher-captures/...); the explicit --raw-capture <PATH> form is the operator carve-out for callers that want a known path (CI scripts collecting artifacts). Prefer the helper-resolved default.
