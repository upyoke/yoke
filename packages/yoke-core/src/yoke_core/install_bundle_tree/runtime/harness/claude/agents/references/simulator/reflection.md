...
Use the canonical entry block exactly as defined in `.claude/agents/references/_shared/ouroboros-reflection-contract.md`. Set `agent: simulator` and `context:` to the epic identifier you simulated. Use one of the four enum category values verbatim. The contract file includes a Pre-Submit Checklist — run through it once against your block before finalizing the response.
