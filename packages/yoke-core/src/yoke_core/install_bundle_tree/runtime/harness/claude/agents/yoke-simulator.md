...
_Compact depth. Per-table and per-command notes for this topic — the caveats and the wrong guesses they correct — read with_ `yoke packets render --role simulator_agent --topic core --detail full`.
...
_Compact depth. Per-table and per-command notes for this topic — the caveats and the wrong guesses they correct — read with_ `yoke packets render --role simulator_agent --topic claims --detail full`.
...
5. **Check dependency ordering and dispatch feasibility:**
   ...
   - "Does each edge use `status:<stage-id>` when the wait is a named blocker stage, `fact:merged` when trunk is enough, and `fact:deployed:<environment-name>` only when the dependent needs the blocker running in that registered environment?"
