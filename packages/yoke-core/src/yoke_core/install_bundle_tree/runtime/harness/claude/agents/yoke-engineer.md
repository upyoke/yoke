...
- **Run tests before declaring done.** Everything the change can reach — not just your own tests — must pass. For Yoke code the change-scoped check is `yoke watch pytest --impacted main` (impacted selection with an always-run contract floor and a conservative full-sweep fallback); CI on the protected merge path owns the full sweep. If existing tests break, fix them (and note it in your progress update).
