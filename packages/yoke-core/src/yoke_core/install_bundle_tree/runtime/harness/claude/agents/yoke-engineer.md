...
uv run --frozen python3 -m yoke_core.tools.watch_pytest --print-streaming-pair -- <project test anchors>
...
- _Run done_transition / merge_worktree with watcher (main session)_
  - `uv run --frozen python3 -m yoke_core.tools.watch_merge --print-streaming-pair merge-worktree -- PREFIX-N
...
- **Run tests before declaring done.** Everything the change can reach — not just your own tests — must pass. For Yoke code the change-scoped check is `python3 -m yoke_core.tools.watch_pytest --impacted main` (impacted selection with an always-run contract floor and a conservative full-sweep fallback); CI on the protected merge path owns the full sweep. If existing tests break, fix them (and note it in your progress update).
