...
yoke claims work acquire --process DOCTOR --project yoke --reason scheduled-run`
  - Reason recommended on acquire, required on release. Pick exactly one target variant. Optional --session-id S is a self-identity assertion that the caller IS the named session; it is not cross-session authority. Steering claims are project-scoped work claims: `yoke claims steering acquire --project P [--doc SLUG] [--reason TEXT]`. Acquire atomically pairs the seat with that document (default `CURRENT-PLAN`) and leaves neither claim behind when either is held elsewhere. Inspect with `yoke claims steering list --project P --active-only`; finish with `yoke claims steering release CLAIM_ID --reason TEXT`, which releases the paired document too; direct release of that document is refused while the seat is active. Stale-session reclaim frees both. The itemless loop is `/yoke steer SLUG [--project P]`: a strategy doc is required (offer to create if absent), the document lock releases before a Blitz can claim it, workers launch CLI-only and item-bound (never `/yoke do`), workers are addressed with `yoke say --item PREFIX-N --stdin` (`--actor LABEL` addresses a human organization member; whole `--session UUID`, never a prefix, only for claim-less recipients) and answer the seat as a ROLE with `yoke say --steering` — never a session id, since that resolves at delivery to whichever seat covers the sender's item, parks when none is live, and is handed to the next seat as one digest on acquire (an itemless sender passes `--steering-scope '{"project_id": N}'). Acquire refuses on any OVERLAPPING live scope, not just an identical one, and names the holder by actor and session. Workers answer with substantive updates only (never progress ticks — the send path refuses those as `body_not_substantive`), and the fleet report arrives appended to the messages this session receives — one combined report covering every live steering claim, available work first, then idle holders, starved delivery, unregistered launches, items landed without close-out, dead waits, and the count of steering messages awaiting a seat; pull it between wakes with `yoke steering report get` (optional `--project P` keeps a single scope).
