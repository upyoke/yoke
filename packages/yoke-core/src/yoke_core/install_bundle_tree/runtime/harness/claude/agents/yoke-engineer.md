...
# The wrapper prints wait_mode and why. background-wake emits the bound background + progress-tail pair; in-turn runs here until the command exits.
# After a background-wake completion: tail -80 <raw-capture> using the helper-resolved path in the output.`
