...
- **Do not Stop after arming Monitor while you hold a work claim.** Monitor wakes resume the current turn; ending it closes the reader and the paired `watch_tail` dies on a broken pipe with no completion record and no later wake. The Stop promised-work gate holds such a Stop rather than allowing it. Expect the block, and expect it to cost you the turn — park first if you need the session quiet. A parked session is the real escape: the gate reads the park before every hold, allows that Stop, and spends no reinjection on it.
...
**Both tiers:**
- **Do not manually poll a running long command.** In `background-wake` the one armed subscription is the progress surface; in `in-turn` the original foreground call is the waiter. The minted `yoke watch tail <progress-capture>` is the only sanctioned Monitor shape — it exits on the completion sentinel, and a re-arm resumes from what it already delivered instead of replaying the backlog, while bare `tail -f` on a watcher capture is denied. One post-completion `tail -80 <raw-capture>` is fine.
