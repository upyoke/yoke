...
- **On Monitor wakes, relay the matched line into your own output.** For merge and deploy, matched lines are actionable terminal errors or the final result; routine progress, retryable warnings, metadata, and no-progress notices are suppressed. Other wrappers may send a `# watch_<kind> digest …` line covering several progress signals; relay it whole, including any `(suppressed N ticks)` suffix. Never substitute filler like "Still waiting."
