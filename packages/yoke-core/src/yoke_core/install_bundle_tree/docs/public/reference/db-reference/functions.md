...
| `claims.steering_scope.acquire` | `None` (handler refuses intersecting live scope) | `yoke_core.domain.handlers.claims_steering_scope.handle_acquire` |
| `claims.steering_scope.release` | `"self_only"` | same handler; ordinary work-claim release by claim id |
| `claims.steering_scope.list` | `None` (project-scoped read) | same handler; project/holder/active filters |
