...
| `merge_queue.landing_pull_request.record` / `merge_queue.landing_pending.mark` / `merge_queue.landing.observe` / `merge_queue.landing_pending.clear` | `None` | Internal, session-optional item functions owned by `yoke merge item`: record the pull request/admission, rate-limit one server GitHub observation across the project's pending landings and return this lane's durable record, then clear it after terminal close-out. |
