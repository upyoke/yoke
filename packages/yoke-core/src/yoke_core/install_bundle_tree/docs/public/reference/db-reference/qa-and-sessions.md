...
- `session_stale_ttl_with_holdings_minutes` (default `1440`) — the minimum tier for a session holding any of those three active resources. It prevents a long foreground command from losing its claim or lock merely because no tool-boundary heartbeat landed. An item holder the fleet probe already reports as `claimed_by_stale` is reclaimed on the probe's short TTL instead; `yoke sessions terminate SESSION-ID --reason R` releases held work claims immediately.
