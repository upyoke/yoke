"""Structural tests for the installed ``api-route.ts`` Pack source.

Companion to ``test_events_ts.py`` (which covers the ``events.ts``
client-side source). Confirms required handlers, validation rules,
...
def api_route() -> str:
    assert _API_ROUTE.is_file(), f"Pack source missing: {_API_ROUTE}"
