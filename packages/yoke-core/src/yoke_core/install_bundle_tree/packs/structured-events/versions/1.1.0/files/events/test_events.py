...
def test_get_system_props_defaults():
    ...
    assert props["project"] == "{{project_name}}"
...
def test_build_event_extra_props_merge():
    ...
    event = build_event(name="Test", kind="audit", event_type="test", **actor)
...
def test_build_event_extra_props_override_system():
    ...
    event = build_event(name="Test", kind="audit", event_type="test", **system)
