"""Structural tests for the installed ``events.ts`` Pack source.
...
the Structured Events Pack — neither compiles the TypeScript.

Run::

    python3 -m pytest events/test_events_ts.py events/test_events_ts_api.py -v
"""
...
def events_ts() -> str:
    assert _EVENTS_TS.is_file(), f"Pack source missing: {_EVENTS_TS}"
...
def events_ts_concat() -> str:
    ...
    for path in _EVENTS_TS_SPLIT:
        assert path.is_file(), f"Pack source missing: {path}"
...
class TestEventsTsSystemProps:
    ...
    def test_system_props_env_wiring(
        self, events_ts_concat: str, needle: str, label: str
    ) -> None:
...
class TestEventsTsSessionProps:
    ...
    def test_session_props_storage(
        self, events_ts_concat: str, needle: str, label: str
    ) -> None:
        _must_contain(events_ts_concat, needle, label)
...
class TestEventsTsAttribution:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "{{project_name}}_attribution",
                "cookie name {{project_name}}_attribution",
            ),
...
class TestEventsTsGracefulDegradation:
    def test_graceful_degradation_comment(self, events_ts_concat: str) -> None:
        _must_contain(
            events_ts_concat, "Graceful degradation", "graceful degradation comment"
        )
...
class TestEventsTsPageViewedExample:
    def test_example_function(self, events_ts_concat: str) -> None:
        _must_contain(
            events_ts_concat, "emitPageViewedExample", "PageViewed example function"
        )

    def test_example_event_type(self, events_ts_concat: str) -> None:
        _must_contain(
            events_ts_concat, "eventType: 'page_view'", "example uses page_view type"
        )

    def test_example_event_kind(self, events_ts_concat: str) -> None:
        _must_contain(
            events_ts_concat, "kind: 'analytics'", "example uses analytics kind"
        )
