...
Every entry in the dedicated test home must belong to the test user. Read-only
files are valid user state; files owned by root or another account are not,
because the test user cannot reliably clear and restore them. This check must
print nothing:

```text
find "$HOME" -xdev ! -user "$(id -un)" -print
```

Repair any reported owner inside the dedicated test home and repeat the check;
do not omit that path from the capture. Never broaden the check to another
login's home.

Seal an adjacent `<golden-path>.probes` document with absolute argv for every
required harness and application state. Login-Keychain probes run through the
GUI Terminal bridge, not SSH. Use macOS's guaranteed `/bin/test` primitive for
filesystem assertions; `/usr/bin/test` is not present on every supported macOS
release. Run the probes before capture and again after a reset. A golden is
accepted only when both runs pass and its manifest remains unchanged.
Apply the golden directory's final owner and mode before generating that
manifest; sealing permissions afterwards records a transient state that every
honest integrity check rejects.
