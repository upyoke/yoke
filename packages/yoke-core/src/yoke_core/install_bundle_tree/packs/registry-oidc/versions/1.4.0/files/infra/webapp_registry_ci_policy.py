...
def delivery_policy_json(
    ...
    *delivery_authority* is the opt-in SSM and artifact grant described by
    :mod:`webapp_registry_delivery_ssm_policy`. ``None`` adds nothing, so a
    project that never opts in keeps exactly the policy it had before.
    """
    repository_arn = (
        ...
        statements.extend(
            [
                ...
                },
                {
                    ...
                    "Effect": "Allow",
                    # Waiter `aws cloudfront wait invalidation-completed` needs
                    # GetInvalidation; CreateInvalidation alone leaves publish red.
                    "Action": [
    ...
    if delivery_authority is not None:
        statements.extend(
            delivery_authority.statements(region=region, account_id=account_id)
        )
    statements.append(
        github_app_private_key_deny_statement(
            region=region,
