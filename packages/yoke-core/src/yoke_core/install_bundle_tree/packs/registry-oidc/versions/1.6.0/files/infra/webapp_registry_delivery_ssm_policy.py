...
#: Systems Manager reserves this prefix for the documents AWS itself
#: publishes, so a document's name decides its ownership.
_AWS_OWNED_DOCUMENT_PREFIX = "AWS-"


def _document_arn(region: str, account_id: str, name: str) -> str:
    """The ARN Systems Manager actually evaluates for this document.

    An AWS-owned public document is owned by no account, and its ARN carries
    an empty account field. Interpolating the project's account there produces
    a grant that can never match: the caller is denied on
    ``arn:aws:ssm:<region>::document/<name>`` while the policy allows
    ``arn:aws:ssm:<region>:<account>:document/<name>``. Because the denial
    names the resource the caller wanted rather than the one granted, the two
    read as the same string at a glance.

    Customer-owned documents keep the account; the prefix is the ownership
    test.
    """
    owner = "" if name.startswith(_AWS_OWNED_DOCUMENT_PREFIX) else account_id
    return f"arn:aws:ssm:{region}:{owner}:document/{name}"
...
def ssm_delivery_statements(
    *,
    ...
    document_arns = [_document_arn(region, account_id, name) for name in document_names]
    return [
        {
...
def delivery_authority_from_config(values: Any) -> DeliveryAuthority:
    ...
    if not isinstance(values, Mapping):
        raise SsmDeliveryConfigError(
            "delivery authority configuration must be a mapping"
        )
    unknown = set(values) - {
        "instance_tags",
        "documents",
    ...
    return DeliveryAuthority(
        instance_tags={
            name: list(tag_values)
            for name, tag_values in _clean_tags(values.get("instance_tags"))
        },
