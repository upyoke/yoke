...
from webapp_runner_fleet_capacity import spot_capacity_policy
...
class WebappRunnerFleetStack(pulumi.ComponentResource):
    ...
    def __init__(
        ...
        self.asg = aws.autoscaling.Group(
            ...
            mixed_instances_policy=spot_capacity_policy(self.launch_template.id, args),
            # Replace a reclaimed instance while its notice still has time left.
        ...
        url_permission = aws.lambda_.Permission(
            "runnerFleetWebhookUrlPermission",
            action="lambda:InvokeFunctionUrl",
            function=self.webhook_function.name,
            principal="*",
