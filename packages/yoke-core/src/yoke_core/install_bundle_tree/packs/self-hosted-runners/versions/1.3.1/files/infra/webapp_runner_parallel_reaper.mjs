...
export async function reapParallelFleet({
  ...
  for (const instanceId of idleHosts) {
    const inherited = previousIdle[instanceId] || state.idle_since || now;
    // A host can finish a very short job and re-register between reaper
    // passes. Its old idle mark then still exists even though this fresh
    // registration proves the host just worked. Never let inherited idle
    // time predate the current ephemeral-runner cycle.
    idleByInstance[instanceId] = Math.max(
      inherited, markers.get(instanceId).at,
    );
  }
  state = {
    ...state,
