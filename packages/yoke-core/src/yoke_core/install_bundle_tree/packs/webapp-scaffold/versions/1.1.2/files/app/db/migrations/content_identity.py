...
def applied_names(conn, history=()) -> set[str]:
    ...
    rows = conn.execute(
        f"SELECT {name_expr}, {version_expr} FROM schema_version"
    ).fetchall()
    by_sequence = {entry.sequence: entry.name for entry in history}
    return {
        str(name) if name is not None else by_sequence[int(version)]
        for name, version in rows
        if name is not None or (version is not None and int(version) in by_sequence)
    }
...
def _ledger_rows(conn, history) -> list[dict]:
    ...
    rows = conn.execute(
        f"SELECT rowid, {name_expr}, {version_expr}, {digest_expr} "
    ...
    result = []
    for rowid, raw_name, raw_version, raw_digest in rows:
...
def _validate_adoption(
    conn, history, manifest, load_module, state_verifiers,
) -> list[dict]:
    rows = [row for row in _ledger_rows(conn, history) if row["entry"] is not None]
    candidates = [
        row for row in rows if row["name"] is None or row["digest"] is None
    ]
    for row in candidates:
        entry = row["entry"]
        expected = module_sha256(entry)
        ...
        verifier = (
            ...
            else getattr(module, "invariants", None)
        )
...
def _record_adoption_receipt(
    ...
    adopted_json = json.dumps(
        [
            {"name": name, "content_sha256": manifest["entries"][name]}
            for name in adopted
        ],
        separators=(",", ":"),
        sort_keys=True,
    ...
    cursor = conn.execute(
        ...
        "source_commit, adopted_by, adopted_entries_json) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (
            manifest["manifest_sha256"], artifact["engine_version"],
            artifact["source_artifact"], artifact["source_sha256"],
            artifact["source_commit"], adopted_by, adopted_json,
        ),
    )
    return {
...
def adopt_from_manifest(
    conn, history, manifest_path, *, running_version, source_commit,
    source_sha256, manifest_sha256, adopted_by, ensure_ledger, load_module,
    ...
    try:
        ...
        raise RuntimeError("Migration state verifiers must be a name/callable map") from exc
    ...
    try:
        ...
        ensure_ledger(conn, history, commit=False)
        candidates = _validate_adoption(
            conn, history, manifest, load_module, verifiers,
        )
        adopted = []
        for row in candidates:
            entry = row["entry"]
            cursor = conn.execute(
                ...
                "WHERE rowid=? AND "
                "(migration_name IS NULL OR content_sha256 IS NULL)",
                (entry.name, manifest["entries"][entry.name], row["rowid"]),
            )
        ...
        receipt = _record_adoption_receipt(conn, manifest, adopted, operator)
        conn.commit()
