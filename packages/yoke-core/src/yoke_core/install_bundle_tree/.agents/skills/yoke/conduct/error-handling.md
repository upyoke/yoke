...
- **Workflow routing.** The conduct loop verifies its registered Epic workflow
  ...
  and derive side effects. Dispatch-chain and unified worktree-creation calls
  are retained internal conduct boundaries, not normal product flow. Explicit
  architect/refine worktrees are preserved; only empty legacy task worktrees
  default to `PREFIX-{N}`. Task specs come from
  `yoke workflow-item epic-task body-get`.
