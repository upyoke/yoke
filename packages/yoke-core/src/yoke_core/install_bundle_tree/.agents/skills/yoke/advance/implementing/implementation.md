...
2. **Honor the resolved File Budget posture.** When
   `_effective_file_budget_policy` is not `optional`, read `## File Budget` from the same
   spec/body before the first write and obey its planned files and
   responsibilities. When it is false, do not require or invent that section;
