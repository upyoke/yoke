...
For `work-claim-conflict`, the right remediation is to coordinate with the holder or wait. For `path-claim-blocked`, follow the `BLOCKED:` / `DIVERGED:` rows to the upstream coordination work item. For `dirty-tracked` / `dirty-untracked`, ask the named holder to commit / stash / drop the files (recipe in the narrative includes their session id), or do that yourself if you are the holder, then retry. Untracked repo-root scratch is not a block. For `worktree-create-failed`, surface the `git worktree add` error verbatim and stop.
