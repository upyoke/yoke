...
plus the instruction you last acknowledged — reaches the seat once, so a
reworded retry deduplicates rather than arriving twice, and a send answering
`Collapsed into an earlier message` did not deliver the body you just sent. A
resume opens a new leg and its completion is delivered, whether or not it hands
you a fresh claim; never release an unfinished lane merely to be heard. Ending a turn sends no
