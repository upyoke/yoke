...
wait is over. Re-run the same `yoke merge item` command with `--result` and
`--verification` then, and it finishes the close-out. Never poll the run. When the resolved flow discharges delivery at
