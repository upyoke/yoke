...
Before writing any changes, complete these mandatory checks and carry the findings into the critique. **Incorporate all survey findings from step 3** — staleness, overlap with active/pipeline/done work items, and codebase drift from recent commits are first-class critique dimensions, not sidebar observations.
...
- **User-provided input inventory (mandatory first check):** Scan the spec for user-voiced content: numbered questions or items from the operator, "I noticed/saw/found" observations, screenshot references, direct quotes, evidence links, "why does X" / "what's the point of" phrasing, and any content that reads as the operator's own words rather than agent-generated prose. List them explicitly in the critique under a "User questions and evidence to preserve" heading. These items must appear verbatim in the enhanced spec — they define what the work item must answer or address.
- **Staleness and overlap (from step 3):** If the survey found codebase drift, overlapping work items, or recently-done work that affects this item, those findings MUST appear in the critique. Recommend descoping, absorbing, dependency-linking, or cancellation as appropriate.
- **Events forensics:** Query item-specific events with `yoke events query --item PREFIX-N` when available; otherwise inspect recent telemetry with `yoke events tail --limit 20`. Use that context to decide whether the artifact quality problem reflects a systemic dispatch/context issue.
...
- **Done-means agent shape:** Any verification command, definition of done, or AC "run this" step must be a guard-permitted shape (`yoke <subcommand>`, `yoke watch pytest -- ...`, `yoke dev run -- python3 -m ...`). Flag and rewrite fenced or backticked `python3 -c` imports of `yoke_core` / `yoke_cli` / `yoke_harness` — readiness `BLOCKED_AGENT_COMMAND_SHAPE` blocks those prescriptions because the import lint refuses them before the command can run.
- **Codebase-reader naming:** Assume future readers of the codebase will NOT have the ephemeral planning artifacts this item was written from. For every proposed new or renamed file, module, helper, test, doc, command, event, config key, symbol, heading, or comment, verify that the name describes current function, purpose, mechanics, or domain role to a repository reader. Flag and rewrite names derived from work items, strategy docs, plan names, initiatives, phases, task/thread numbers, AC/FR identifiers, branches, worktrees, or implementation batches unless that identifier is itself a runtime/domain concept.
...
- **Future-concept pull-forward lens:** If the item adds or changes `actor_id`, `session_id`, `heartbeat_at`, ownership, leases, claims, approvals, overrides, evidence, run records, execution journals, compiled packets, route-around facts, resource locks, or shared-state coordination, decide whether that surface is the smallest honest v0 of a later end-state primitive. If yes, the refined spec must name the primitive, current consumers, and explicit out-of-scope future surface. If no, the spec must name the deletion or absorption target. Flag any plan that creates a local temporary surface while an existing primitive (`path_claims`, coordination claims, events, actors, phase runs, execution journal, compiled packets) can be consumed instead.
...
- End-to-end completeness: Can the operator actually use and experience the result after this work item ships? If the spec describes building a feature but doesn't mention wiring it into the UI, CLI, or workflow where users encounter it, that's a gap. If it changes behavior but doesn't mention updating the docs/help text/error messages that describe that behavior, that's a gap. Trace the user's journey from trigger to outcome and flag every missing step.
- Missing requirements: Would a reasonable person expect outcomes that the work item doesn't mention? Surface them aggressively — error handling, input validation, user-facing messaging, edge cases obvious from the problem statement, integration points with existing systems. The question is not "did the operator write this down" but "would the operator be surprised if this wasn't done."
...
- Migration entry shape: When the item's DB claim declares `mutation_intent='apply'` (see the `db_mutation_profile` JSON-nested-field schema in your packet) and the profile carries one or more `migration_modules`, check the spec describes an entry in the ordered history — named `NNNN_slug.py`, permanent, guarded so it is safe to re-run, and applied by the boot converge rather than by any step the item performs. Reject any AC that says the module is deleted after it runs, that waits on evidence the module reached every install, or that has the item apply to an authoritative database: modules are never deleted, and applying is not the item's job.
...
- Sizing check: Does the scope match the selected workflow? If the spec has
  5+ FRs touching 3+ files in different subsystems, it should probably use a
  task-decomposing workflow. If it has 7+ FRs or 10+ ACs, flag it as likely
  needing task decomposition.
...
- Generated-task alignment: If `epic_tasks` rows already exist, do they still match the plan and worktree grouping?
...
**QA method coverage**
- Does the verification design name the observable Browser, command, or
  machine behavior that must be proved?
- Prefer an existing project QA plan when its cases cover the work. Otherwise
  require an explicit method-backed case with instructions, expected outcome,
  and method configuration.
