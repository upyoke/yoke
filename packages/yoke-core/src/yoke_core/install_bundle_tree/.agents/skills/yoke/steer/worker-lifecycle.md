...
it when a QA stage needs it or its own item-scoped QA is accepted. Once that
member's scoped QA passes or is waived, a selected flow without run QA closes
that final member after its own final production QA is accepted, even while
sibling QA keeps the run executing. Required run QA holds all final members
through shared QA and run success. Automatic close-out ends an otherwise empty
...
anything else is expected to re-park before it goes quiet again. A holder that goes quiet WITHOUT that park is treated as gone —
