...
it when a QA stage needs it or its own item-scoped QA clears. Once that
member's scoped QA passes, is waived, or is explicitly discharged by
`post_deploy_no_obligation`, a selected flow without run QA closes
