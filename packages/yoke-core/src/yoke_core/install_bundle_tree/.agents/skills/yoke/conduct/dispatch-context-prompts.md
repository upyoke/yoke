...
# For each task in _task_ids, dispatch simultaneously with the rendered descriptor:
 ...
 Ephemeral URL: {_ephemeral_url}
	 ...
	 DB MIGRATION PROTOCOL: This task involves schema changes. You MUST follow the migration protocol in your agent definition (## DB Schema Changes section). Key steps: python3 -m yoke_core.domain.backup BEFORE any DDL, update python3 -m yoke_core.cli.db_router projects + python3 -m yoke_core.engines.doctor + db-reference.md + domain python3 -m yoke_core.cli.db_router wrappers AFTER. The lint hook will block unacknowledged DDL in python3 -m yoke_core.cli.db_router query.}
