...
INSTALLER_ADAPTERS: List[AdapterEntry] = [
    ...
    AdapterEntry(function_id="project.register.run", cli_invocation="yoke project register REPO_ROOT --project-id N", notes="Machine-local checkout -> project id mapping."),
    ...
    AdapterEntry(function_id="projects.capability_secret.set", cli_invocation="yoke projects capability secret set --project NAME --cap-type TYPE --key KEY VALUE", notes="Stores capability secrets; GitHub App private keys use app_private_key while repository access uses binding rows. aws-admin secrets and ssh.private_key are written to the local machine under ~/.yoke/secrets/capability-secrets. Direct VALUE is the default, with --value-file/--value-stdin for scripted inputs."),
