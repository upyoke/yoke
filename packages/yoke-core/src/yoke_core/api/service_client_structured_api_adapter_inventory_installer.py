...
INSTALLER_ADAPTERS: List[AdapterEntry] = [
    ...
    AdapterEntry(function_id="project.artifacts.refresh", cli_invocation="yoke project artifacts refresh [REPO_ROOT] --project NAME [--apply|--verify]", notes="Client-local full-preflight reconciliation of manifest-owned workflows, generated deployment references, ops programs, and static infrastructure. The checkout project binding must match before planning; a verified repository binding additionally requires an exact live Git origin. Fresh authority comes from the registered projects.artifacts.render read over the active transport. DB-backed project-policy artifact_refresh.enabled=false plus a reason returns a clean post-identity no-op for project-owned release factories."),
