...
INSTALLER_ADAPTERS: List[AdapterEntry] = [
    ...
    AdapterEntry(function_id="project.artifacts.refresh", cli_invocation="yoke project artifacts refresh [REPO_ROOT] --project NAME [--apply|--verify|--adopt-existing]", notes="Client-local full-preflight reconciliation of manifest-owned workflows, generated deployment references, ops programs, and static infrastructure. The checkout project binding must match before planning; a verified repository binding additionally requires an exact live Git origin. Fresh authority comes from the registered projects.artifacts.render read over the active transport. A legacy rendered checkout with no manifest can explicitly adopt current managed-path bytes before ordinary preview/apply reconciliation. DB-backed project-policy artifact_refresh.enabled=false plus a reason returns a clean post-identity no-op for project-owned release factories."),
    _read_entry(function_id="projects.artifacts.render", cli_invocation="yoke project artifacts refresh [REPO_ROOT] --project NAME", notes="Server-safe render from packaged webapp templates plus the project's DB-backed settings and generic applicability policy. The client wrapper applies the returned secret-free bundle; Pulumi operator state stays on the exact stack-config surface."),
