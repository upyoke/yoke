...
INSTALLER_ADAPTERS: List[AdapterEntry] = [
    ...
    AdapterEntry(function_id="projects.capability_secret.set", cli_invocation="yoke projects capability secret set --project NAME --cap-type TYPE --key KEY VALUE", notes="Stores GitHub tokens as Yoke-owned capability_secrets literals; aws-admin secrets and ssh.private_key are written to the local machine under ~/.yoke/secrets/capability-secrets. Direct VALUE is the default, with --value-file/--value-stdin for scripted inputs."),
