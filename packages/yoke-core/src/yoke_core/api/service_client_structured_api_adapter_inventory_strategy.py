...
STRATEGY_ADAPTERS = [
    ...
    AdapterEntry(function_id="strategy.doc.create", cli_invocation="yoke strategy doc create <slug> --content-file PATH --target-root PATH", notes="Create a new DB-authoritative strategy doc, then render the tracked .yoke/strategy/ view into target_root."),
    AdapterEntry(function_id="strategy.doc.replace", cli_invocation="printf '%s' \"$CONTENT\" | yoke strategy doc replace <slug> --stdin --base-updated-at TS --target-root PATH", notes="Process-claim-gated CAS write (STRATEGIZE/FEED conflict group; base_updated_at from doc get); auto-renders the full tracked strategy view into target_root; shrink guard bypass via --force."),
    AdapterEntry(function_id="strategy.render.run", cli_invocation="yoke strategy render --target-root PATH", notes="Writes the project-scoped tracked .yoke/strategy/ rendered view from the DB authority (idempotent headers); target_root resolves client-side."),
