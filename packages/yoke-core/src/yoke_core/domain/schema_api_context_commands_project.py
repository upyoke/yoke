...
PROJECT_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "secret values. Every run opens with `yoke pulumi exec: rendering "
            "<checkout> @ <revision> (<infra/ state>)` on stderr — that line "
            "is the tree the program was read from, so read it before "
            "trusting an unchanged count, and note that uncommitted work "
            "under `infra/` is rendered and reported rather than ignored. "
