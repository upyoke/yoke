...
def __getattr__(name: str):
    if name in {"TOOL_LATENCY_FIELDS", "tool_latency_rows"}:
        from yoke_core.domain.hook_overhead_tool import (
            TOOL_LATENCY_FIELDS,
            tool_latency_rows,
        )

        return (
            TOOL_LATENCY_FIELDS if name == "TOOL_LATENCY_FIELDS" else tool_latency_rows
        )
