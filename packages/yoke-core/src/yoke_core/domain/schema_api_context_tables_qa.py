...
QA_TABLES: dict[str, dict] = {
    "qa_requirements": {
        ...
        "notes": (
            ...
            "except for kinds this case's OWN `runner_id` provisions where it "
            "executes (`qa_method_capabilities."
            "RUNNER_HOST_PROVISIONED_CAPABILITY_KINDS`: `browser_substrate` and "
            "`agent_mission` install `browser-control` on the machine that runs "
            "the case). No project row proves or withholds those, so admission "
            "admits them and that machine provisions them on first use; adding "
            "an empty `browser-control` project row is a stale workaround, not "
            "the contract. The exemption is per runner, never per kind: a "
    ...
    "qa_runs": {
        ...
        "notes": (
            ...
            "`verdict`. Agent `undetermined` requires `verdict_reason` plus a "
