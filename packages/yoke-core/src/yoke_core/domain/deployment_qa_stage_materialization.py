...
def _member_plans(subject: Mapping[str, Any]) -> list[dict[str, Any]]:
    snapshot = subject.get("member_snapshot")
    if not isinstance(snapshot, Mapping):
        return []
    return [
        dict(plan) for plan in snapshot.get("plans") or [] if isinstance(plan, Mapping)
    ]
...
def _selected_plans(
    ...
    *,
    ...
    frozen = [*_flow_plan(subject), *_member_plans(subject)]
...
def _existing_rows(
    ...
    *,
    ...
    return query_rows(
        ...
        "AND COALESCE(deployment_member_item_id,0)=%s AND plan_id=%s ORDER BY id",
        (run_id, stage_name, member_item_id or 0, int(plan_id)),
    )
...
def materialize_deployment_qa_stage(
    ...
    *,
    ...
    admitted = member_requirements(subject)
