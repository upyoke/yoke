...
Blocks hand-quoted JSON to ``service_client``, registry-covered CLI wrapped
in shell choreography, and unregistered subcommand paths. Designed-in
adapter use (read wrapping, ``cat file | adapter --stdin``, ``--help``)
stays allowed. ``# lint:no-shell-json-payload-check`` is audit-only.
"""
...
# Adapter index: ``"<module> <sub-path>" -> function_id``; longest module first.
...
def _has_hand_quoted_json_payload(command: str) -> bool:
    return bool(
        _PRINTF_PIPE_PAYLOAD_RE.search(command)
...
def _find_module_in_command(command: str) -> Optional[Tuple[str, str]]:
    """Return ``(module, tail)`` for the longest ``python -m <module>`` hit."""
...
def _find_registry_covered_adapter(
    ...
    """Return a registry adapter hit, or a domain-only miss, or ``None``."""
        ...
        if not upstream:
            if is_read_shape_function(hit.function_id):
                if is_read_wrapping(tail, mktemp_vars=mvars):
                    return None
            elif is_best_effort_wrapping(tail, mktemp_vars=mvars) or (
                hit.function_id not in _NO_CONSUMER_ALLOWANCE_FUNCTIONS
...
_UPSTREAM_MKTEMP_RE = re.compile(r"\b(?P<name>[A-Za-z_]\w*)=\$\(\s*mktemp[^)]*\)\s*")
...
def _emit_denial(payload: dict, reason: str, *, outcome: str = "denied") -> None:
    ...
    try:
        emit_denial_event(
            hook="lint-shell-quoted-function-payload",
            tool="Bash",
...
def evaluate(record: HookContext) -> HookDecision:
    ...
    if reason is None:
        ...
        if note:
            ...
            return HookDecision(
                outcome=Outcome.WARN, message=note, audit_fields={"note": note}
            )
        return HookDecision(outcome=Outcome.NOOP, next=Next.CONTINUE)
