...
RELEASE_WAIT_RETENTION_TEACHING = (
    ...
    "automatically when the completion-flow run succeeds, without another "
    "wake; the close-out also ends an otherwise empty holder session. A stage that "
    ...
    "item-scoped stage is accepted, re-park while its completion flow "
    "finishes; do not re-run merge solely for that acceptance. A delivery "
