...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "`yoke say --item PREFIX-N --stdin` (`--actor LABEL` addresses a "
            "human organization member; whole `--session UUID`, "
            "never a prefix, only for claim-less recipients) and answer "
            "the seat as a ROLE with `yoke say --steering` — never a session "
            "id, since that resolves at delivery to whichever seat covers "
            "the sender's item, parks when none is live, and is handed to "
            "the next seat as one digest on acquire (an itemless sender "
            "passes `--steering-scope '{\"project_id\": N}'). Acquire "
            "refuses on any OVERLAPPING live scope, not just an identical "
            "one, and names the holder by actor and session. Workers answer "
            ...
            "items landed without close-out, dead waits, and the count of "
            "steering messages awaiting a seat; pull it between "
