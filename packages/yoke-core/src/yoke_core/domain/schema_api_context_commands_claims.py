...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "working block, context-budget pause). The SessionEnd guard "
            "(yoke_core.domain.sessions_lifecycle_destructive_guard."
            "evaluate_destructive_end) defers release when activity "
            "signals look alive (recent heartbeats, in-flight chainable "
            "checkpoint) — explicit release is the canonical handoff "
            "shape. For terminal handoffs (handoff-to-polish, "
            "handoff-to-usher, finalize-exit), the lifecycle transition "
            "itself releases — do not use this recipe there. Pair with "
            "a Progress Log entry so the fresh session inherits resume "
            "context."
        ),
    },
    {
