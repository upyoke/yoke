...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            "Reason recommended on acquire, required on release. Pick exactly "
            "one target variant. Optional --session-id S is a self-identity "
            "assertion that the caller IS the named session; it is not "
            "cross-session authority. Steering claims "
            "are scope-serialized work claims: `yoke claims steering acquire "
            "--project P [--doc SLUG | --plan-doc SLUG] [--reason TEXT]`. "
            "`--doc` narrows the seat to that document's linked items and "
            "locks it; `--plan-doc` locks the standing plan while the seat "
            "still covers the whole project (the project-level shape); "
            "neither flag covers the project and locks nothing. Passing both "
            "refuses. Overlapping scopes refuse and name the holder's actor "
            "and machine; two documents coexist. Inspect with "
            "`yoke claims steering list --project P --active-only`; finish with "
            "`yoke claims steering release CLAIM_ID --reason TEXT`, which "
            "releases the paired document too; direct release of that document "
            "is refused while the seat is active. Stale-session reclaim frees "
            "both. The itemless loop is "
            "`/yoke steer [SLUG] [--project P ...]`: an omitted slug resolves "
            "to CURRENT-PLAN per named project and is never a question to ask, "
            ...
            "and item-bound (never `/yoke do`). The single-recipient `yoke say --item PREFIX-N --stdin` form is insufficient for a substantive peer request. Such a request addresses its worker "
            ...
            "(`--actor LABEL` reaches a human member.) Workers answer the seat by ROLE with "
            "`yoke say --steering` — never a session id: it resolves at "
            "delivery, parks with no live seat, and drains to the next seat "
            "on acquire, which refuses any OVERLAPPING live scope. Send, "
            "preview, get and list all carry that durable `steering_recipient` "
            "(`awaiting_seat` with its scope, `delivered` naming the seat, or "
            ...
            "one report per work leg — the episode you send it in, under the claim you hold then — reaches the seat once even when a reworded retry follows, while a completion you were resumed to do is its own leg and is delivered whether or not the resume handed you a fresh claim; a resume that never crossed a session end is the one shape still collapsed, and it says so. "
            ...
            "something it would act on; progress ticks belong in this "
            "session's own visible output. That is coordination advice, "
