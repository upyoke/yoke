...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "`acknowledged`), so a queued report is never a zero-recipient "
            "failure. Its address is the item you hold, else the one you last "
            "held in this session, so a DONE still resolves after close-out "
            "released the claim; send it before releasing a claim you still "
            "hold, and one terminal report per session and item reaches the "
            "seat once even when a reworded retry follows. Every worker reaches the "
