...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "--project P [--doc SLUG] [--reason TEXT]`. With `--doc` the seat "
            "covers exactly that document's linked items and takes its lock "
            "atomically; without it the seat covers the whole project and "
            "locks nothing. Overlapping scopes refuse and name the holder's "
            "actor and machine; two documents coexist. Inspect with `yoke "
            ...
            "both. The itemless loop is `/yoke steer SLUG [--project P]`: a "
            "strategy doc is required (offer to create if absent), its lock "
            "releases before a Blitz can claim it, workers launch CLI-only "
            "and item-bound (never `/yoke do`), workers are addressed with "
            "`yoke say --item PREFIX-N --stdin` (`--actor LABEL` reaches a "
            "human member; a whole `--session UUID` only for claim-less "
            "recipients) and answer the seat by ROLE with "
            "`yoke say --steering` — never a session id: it resolves at "
            "delivery, parks with no live seat, and drains to the next seat "
            "on acquire, which refuses any OVERLAPPING live scope. Its "
            "address is the item you hold, else the one you last held in this "
            "session, so a DONE still resolves after close-out released the "
            "claim; send it before releasing a claim you still hold, and "
            "one terminal report per session and item reaches the seat once "
            "however many routes carried it. A steering-LAUNCHED session "
            "sends far less than that: the Stop "
            "hook already delivers the last assistant text of each of its "
            "turns to the seat — the turn after close-out included — so its "
            "turn-end text IS the report and must never be re-sent with "
            "`yoke say`. It carries only a turn naming something to act on; "
            "one ending in a wait or status note is recorded as "
            "`SteeringReportSkipped`, not delivered. An operator-launched or "
            "operator-opened session is not relayed and reaches the seat "
            "with `yoke say --steering` when it must act. "
            "Message another session only for something it would act on; progress "
            "ticks belong in this session's own visible output. That is coordination "
            "advice, not a send-path refusal. The fleet report arrives appended "
            ...
            "items landed without close-out, dead waits, and mail "
            "awaiting a seat; pull it between "
