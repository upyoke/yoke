...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "address is the item you hold, else the one you last held in "
            "this session, so a DONE still resolves after close-out released "
            "the claim; send it before releasing a claim you still hold, and "
            "one terminal report per session and item reaches the seat once "
            "however many routes carried it. A "
            "steering-LAUNCHED session sends far less than that: the Stop "
            "hook already delivers the last assistant text of each of its "
            "turns to the seat — the turn after close-out included — so its "
            "turn-end text IS the report and must "
            "never be re-sent with `yoke say`. An operator-launched or "
            "operator-opened session is not relayed and reaches the seat "
            "with `yoke say --steering` when the seat must act. Answers "
            "carry substantive updates only "
