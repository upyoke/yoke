...
CLAIMS_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "`/yoke steer [SLUG] [--project P ...]`: an omitted slug "
            "resolves to CURRENT-PLAN per named project and is never a "
            "question to ask, the resolved doc is read before any steering "
            "action, only a genuinely missing doc reaches the offer-to-create "
            "gate, its lock "
            ...
            "even when a reworded retry follows. Every worker reaches the "
            "seat deliberately with `yoke say --steering`; ending a turn "
            "sends no Fleet message. "
