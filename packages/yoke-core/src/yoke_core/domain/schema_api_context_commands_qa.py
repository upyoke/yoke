...
QA_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "carry verdict (pass/fail/undetermined/error), verdict_reason "
            "(required for undetermined; agent outcomes also need linked evidence), "
            "execution_status (capture outcome), and raw_result (result payload). "
    ...
    },
    {
        ...
        "notes": (
            ...
            "qa_runs row including verdict, verdict_reason, execution_status, raw_result, "
    ...
    },
    {
        ...
        "notes": (
            ...
            "complete verdict batch. An agent_mission bundle stays owned by "
            "the main agent, which dispatches atomic walkers, handles human "
            "gates between walker turns, and chooses the final verdict. Only "
            "agent undetermined requires attached evidence, halts the item, "
            "and creates owner/operator Inbox work. A case that never ran uses "
            "the failed/blocked outcome instead. Waiting runs resume from the same cursor, while "
    ...
    },
    {
        ...
        "notes": (
            ...
            "The stage must be the run's active pinned schema-2 QA stage. A "
            "stage is credited only by requirements bound to its own stage "
            "name, and an item-scoped stage only by ones bound to the member "
            "too: item scope requires --member, run scope omits it, and the "
    ...
    },
    {
        ...
        "notes": (
            ...
            "qa.run.complete / qa.artifact.add evidence writes. An ambient "
            "session is required. A Browser case names exactly ONE subject "
            "— its item, or its deployment run with item_id NULL — so the "
            "runner and qa.browser_context.get carry item_id OR "
            "deployment_run_id; never assume a case has an item. "
            "This is not the form that credits a deployment QA stage: it "
            "credits the requirement's existing binding, so release-time stage "
            "evidence goes through `yoke qa plan run --deployment-run-id RUN "
            "--stage STAGE --member PREFIX-N`. "
            ...
            "including a direct run-attached row; a run-and-member-bound "
            "case also gets `DEPLOYMENT_RUN_ID` and `DEPLOYMENT_MEMBER_REF`, "
            "so its command names its own subject rather than a literal that "
            "is stale next run. `method_config.command` "
