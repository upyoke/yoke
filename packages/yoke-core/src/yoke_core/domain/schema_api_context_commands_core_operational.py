...
OPERATIONAL_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "function id predicts its adapter name. A read prints its answer "
            "whole, at any size — nothing decides what to print by looking "
            "at length. Only a named field of a named mutation that restates "
            "state you did not ask it to change condenses to "
            "`<not printed: N bytes … — --json prints the whole envelope>`, "
            "today just `sessions.touch`'s `session` row, with the field "
            "named on stderr; an unlisted case stays long rather than going "
            "missing. The receipt prints after the command has run, so "
            "`--json` is a flag for an invocation that needs the whole "
            "envelope, never a reason to repeat a write you already made. "
    ...
    },
    {
        ...
        "recipe": (
            ...
            "git -C $(git rev-parse --show-toplevel)/.worktrees/PREFIX-N "
            "rev-parse HEAD\n"
            "yoke github-actions failed-log <repo> <run-id> "
            "--project <project>"
        ),
        "notes": (
            ...
            "(bearer-token REST). After a failed CI gate the failed-log "
            "recipe above reads EVERY failed job; omit the run id and pass "
            "`--workflow ci.yml` with optional `--head-sha` resolved from "
            "the lane checkout instead. Each failed job — every matrix shard "
