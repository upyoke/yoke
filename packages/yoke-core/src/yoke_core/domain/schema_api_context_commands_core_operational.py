...
OPERATIONAL_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "trimmed, never truncated: one bulky value — a whole stored row "
            "returned for a one-field stamp, or the argument list you just "
            "passed — prints as `<omitted: N bytes … — rerun with --json>` "
            "while every small fact beside it survives, the omission is "
            "named on stderr, and `--json` and refusals are never "
            "compacted. "
