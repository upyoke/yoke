...
OPERATIONAL_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "function id predicts its adapter name. A bare receipt is "
            "trimmed, never truncated: one bulky value with nothing "
            "actionable in it — a whole stored row returned for a one-field "
            "stamp, or the argument list you just passed — prints as "
            "`<not printed: N bytes … — --json prints the whole envelope>` "
            "while every small fact beside it survives and the marking is "
            "named on stderr. Every failure, recovery, and authority fact "
            "prints in full at any size, nested ones included. The receipt "
            "prints after the command has run, so `--json` is a flag for an "
            "invocation that needs the whole envelope, never a reason to "
            "repeat a write you already made; `--json` and refusals are "
            "never compacted. "
    ...
    },
    {
        ...
        "notes": (
            ...
            "(bearer-token REST). After a failed CI gate, read EVERY failed "
            "job with `yoke github-actions failed-log <repo> <run-id> "
            "--project <project>` or omit the run id and pass "
            "`--workflow ci.yml` with optional `--head-sha` resolved from "
            "the lane checkout; each failed job — every matrix shard — "
            "gets its own labelled block with job id, GitHub job URL, and "
            "its own `--tail-lines` tail, and a job whose log is "
            "unavailable is named with the reason rather than dropped, so "
            "never infer the other shards' failures from one of them. "
            "Never expand a visible short SHA by "
