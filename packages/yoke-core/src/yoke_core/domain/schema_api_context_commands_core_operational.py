...
OPERATIONAL_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "(bearer-token REST). After a failed CI gate, read failed-step "
            "output with `yoke github-actions failed-log <repo> <run-id> "
            "--project <project>` or omit the run id and pass "
            "`--workflow ci.yml` with optional `--head-sha` resolved from "
            "the lane checkout. Never expand a visible short SHA by "
