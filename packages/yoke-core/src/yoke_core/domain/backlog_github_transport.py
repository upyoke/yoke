...
``_github_auth_available`` auth probe name, and the ``_sync_task_children`` dispatch
...
def _sync_task_children(
    ...
    *,
    ...
    """Sync generated task issues and dispatch chains when enabled."""
    if not enabled:
        return 0
    try:
