"""Backlog GitHub transport — PAT availability gate + epic-children sync hook.

Yoke does NOT use the ``gh`` CLI; all GitHub access goes through the
typed :mod:`yoke_core.domain.github_rest` surface plus
:mod:`yoke_core.domain.backlog_github_label_sync_rest`. This module
hosts the two thin helpers that survived the argv-shim purge —
``_dry_run`` (env-var gate) and ``_pat_available`` (auth-resolution
probe) — plus the ``_sync_epic_children`` dispatch wrapper used by
``sync_item``.
"""
...
def _pat_available(project: str) -> bool:
    """Return True iff the project has resolvable GitHub PAT auth.
