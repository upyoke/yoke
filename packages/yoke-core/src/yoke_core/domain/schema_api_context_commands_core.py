...
CORE_COMMANDS: list[dict] = [
    ...
    },
    {
        ...
        "notes": (
            "Multi-field returns one value per line in field order. The "
            "``items.get.run`` JSON result nests stored values under "
            "``result.fields`` (e.g. ``result.fields.db_mutation_profile``); "
            "the wrong guess of a top-level ``db_mutation_profile`` on that "
            "result always reads empty. Valid "
    ...
    },
    {
        ...
        "notes": (
            ...
            "Satisfaction accepts status:<stage-id> (status:done and "
            "status:implemented remain valid), fact:merged, or "
            "fact:deployed:<environment-name>. A stage id is validated "
            "against the blocker's pinned workflow and a refusal lists "
            "the stages that version has. Choose merged "
