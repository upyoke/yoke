...
PROJECT_CAPABILITIES_TABLE: dict[str, object] = {
    ...
    "notes": (
        ...
        "<name> -- <preview|refresh|import|up ...>`; `up` is "
        "local/operator-only and requires both `--yes` and `--non-interactive`. "
        "Local AWS secrets come from machine capability files, GitHub authority "
        "follows the selected service binding, and Actions keeps ambient OIDC. "
