    ...
    **OUROBOROS_TABLES,
    ...
    "item_dependencies": {
        ...
        "notes": (
            ...
            "`classification` column on this table. satisfaction accepts "
            "'status:<stage-id>' (including 'status:done' and "
            "'status:implemented'), 'fact:merged', or "
            "'fact:deployed:<environment-name>'; a stage must exist in the "
            "blocker's pinned workflow version or authoring lists the "
            "stages that version has. deployed facts require a "
            "succeeded run for the blocker's project/environment whose "
            "carried_work names the blocker. "
    ...
    "events": {
        ...
        "notes": (
            ...
            "`harness_sessions` columns + `session_tool_calls`; probe "
            "filtering, completed-work proof, native turn-end cause, "
            "and the automatic-resume budget read the "
            "`harness_sessions` recovery columns "
            "(`session_recovery_facts`); the Stop-hold ceiling reads "
            "`session_promised_work_holds`; a refused call's outcome "
            "reads `session_tool_calls.outcome`; "
