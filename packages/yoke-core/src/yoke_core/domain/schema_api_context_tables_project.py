...
PROJECT_TABLES: dict[str, dict] = {
    ...
    "environments": {
        ...
        "notes": (
            ...
            "JSON `settings` column. `url` is the origin authorized to answer "
            "for the environment and is written by `projects.environment.create` "
            "/ `projects.environment.update` (`yoke projects environment "
            "create|update --url`); environment-settings merge does not write "
            "it. First discover metadata-only names with "
