...
PROJECT_TABLES: dict[str, dict] = {
    ...
    "project_capabilities": {
        ...
        "notes": (
            ...
            "Canonical non-sensitive settings access is `yoke projects capability-settings get --project <slug> --cap-type <type>`, except that capability listings may include `pulumi-state` and its aggregate settings read is closed. Declare an exact `pulumi-state.stacks` entry through the typed capability-settings merge surface, initialize that declared stack through `yoke pulumi exec`, and fetch its exact rendered program with `yoke projects pulumi-stack-config get --project <slug> --stack <name> --output <owner-only-file>`. "
            ...
            "get/full-set/operator-state merges are closed. Aggregate-read callers "
            "must use typed declaration merge, stack initialization, or exact "
            "stack-config fetch according to their intent. Every environment "
            ...
            "For a new explicitly declared stack, the client-local/source-dev "
            "bootstrap is `yoke pulumi exec --project <slug> --stack <name> -- "
            "init --secrets-provider 'awskms://<kms-alias>?region=<region>'`. "
            "It materializes a 0700 scratch workspace, requires the exact "
            "`stacks` declaration, and persists typed operator-state; no "
            "repo-local infrastructure checkout is required. Then use `yoke "
            "pulumi exec --project <slug> --stack <name> -- "
