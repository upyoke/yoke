...
PROJECT_TABLES: dict[str, dict] = {
    ...
    "project_capabilities": {
        ...
        "notes": (
            ...
            "remain binding-owned and are refused by the generic writer. "
            "`pulumi-state.stack_state` is also closed on generic get/full-set/"
            "merge surfaces; top-level non-sensitive pulumi-state merges remain "
            "available and return changed paths only. Fetch one stack through "
            "registered `projects.pulumi_stack_config.get` / `yoke projects "
            "pulumi-stack-config get --project <slug> --stack <name> --output "
            "<file>`, then use typed local `yoke pulumi exec --project <slug> "
            "--stack <name> -- <preview|refresh|import ...>`. The "
