...
PROJECT_TABLES: dict[str, dict] = {
    ...
    "project_capabilities": {
        ...
        "notes": (
            ...
            "generic get/full-set/operator-state merges are closed. The admin "
            "registered stack-config result is metadata-only; `yoke "
            "projects pulumi-stack-config get --project <slug> --stack <name> "
            "--output <file>` fetches the body via no-store into a 0600 file. "
            "Then use `yoke pulumi exec --project <slug> "
            "--stack <name> -- <preview|refresh|import ...>`; local AWS secrets "
            "come from machine capability files, GitHub authority follows the "
            "selected service binding, and Actions keeps ambient OIDC. The "
