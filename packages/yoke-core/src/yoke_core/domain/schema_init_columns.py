...
def apply_additive_schema(conn: Any) -> None:
    ...
    _add_column_if_not_exists(
        conn, "projects", "github_sync_mode", github_sync_mode_column_sql()
    )
    ...
    _add_column_if_not_exists(
        conn, "items", "source", f"TEXT NOT NULL DEFAULT '{DEFAULT_ITEM_ACTOR_ID}'"
    )
    conn.commit()
    ...
    _add_column_if_not_exists(
        conn, "items", "architecture_impact", "TEXT NOT NULL DEFAULT 'none'"
    )
    conn.commit()
...
def apply_legacy_data_migrations(conn: Any) -> None:
    ...
    conn.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_items_project_sequence ON items(project_id, project_sequence)"
    )
    conn.commit()
    ...
    conn.execute(
        "UPDATE items SET architecture_impact = 'none' WHERE architecture_impact IS NULL OR architecture_impact = '' OR architecture_impact = 'null'"
    )
    conn.commit()
