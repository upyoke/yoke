...
from yoke_core.hooks.decision_render import merge_allow_stdout
    ...
    *,
    ...
    if controls.final_outcome != "deny" and extra_context:
        stdout = merge_allow_stdout(
            stdout,
            _context_stdout(extra_context, event_name),
            event_name,
        )
    if stdout:
...
def _context_stdout(context: str, event_name: str) -> str:
    from yoke_harness.hooks.decision_render import render_context_stdout

    return render_context_stdout(context, event_name)
