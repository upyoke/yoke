...
CLI usage::
    ...
    python3 -m yoke_core.hooks.sessions_cli <subcmd> [args...]

Exit codes: 0 success, 1 error/not-found, 2 usage error.

Behavior lives in responsibility-named siblings:

- :mod:`yoke_core.hooks.sessions_focus` — focus/precondition
  helpers + identity/format primitives.
- :mod:`yoke_core.hooks.sessions_event_emit` — event emission
...
from yoke_core.hooks.sessions_inventory import (  # noqa: F401
    cmd_list,
    cmd_stale,
)
from yoke_core.hooks.sessions_lifecycle import (  # noqa: F401
    cmd_begin,
    cmd_end,
    cmd_get,
    cmd_touch,
...
Worked example:
  ...
  python3 -m yoke_core.hooks.sessions_cli who-claims YOK-N --current-episode
