        ...
        if len(run_ids) > 1:
            return ContinueResult(
                ...
                error=(
                    ...
                    f"more than one prepared run ({', '.join(run_ids)}); "
                    "cancel the runs that no longer apply with `yoke "
                    "deployment-runs terminalize RUN-ID --disposition "
                    "cancelled --reason TEXT`"
                ),
                error_code="ambiguous_prepared_run",
            )
        run_id = run_ids[0]
