...
def hc_project_gh_auth(conn, args: DoctorArgs, rec: RecordCollector) -> None:
    ...
    try:
        ...
        intentional_disabled = False
        if isinstance(err, MissingCapability):
            try:
                intentional_disabled = not github_sync_enabled(
                    proj_id,
                    conn=conn,
                )
...
def hc_project_deploy_flows(conn, args: DoctorArgs, rec: RecordCollector) -> None:
    ...
    if rows:
        ...
        rec.record(
            "HC-project-deploy-flows",
            f"Deployment flow state ({args.project})",
            "PASS",
            detail,
        )
    else:
...
def hc_project_health(conn, args: DoctorArgs, rec: RecordCollector) -> None:
    ...
    try:
        ...
        rec.record(
            "HC-project-health",
            f"Production health ({args.project})",
            "WARN",
