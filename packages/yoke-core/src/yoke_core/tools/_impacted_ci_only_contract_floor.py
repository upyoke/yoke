...
item-reference construction policy, and schema packet line budgets mirror
``ci_repo_contracts`` concerns that have no import edge from everyday lane
edits. Packet budgets are the sharpest case: a packet is rendered from live
``--help`` probes and live schema, so editing one command's usage string can
push a role over its line budget from a module the budget test never imports.
