...
STRATEGY_INGEST_USAGE = (
    "yoke strategy ingest [SLUG ...] [--dry-run] [--commit MSG] "
...
def strategy_ingest(args: List[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  yoke strategy ingest MASTER-PLAN --commit \"strategy: ...\""
            "  # write-back + stage + commit in one step"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--commit", dest="commit_message", default=None, metavar="MSG",
        help=(
            "After write-back + re-render, stage the changed .yoke/strategy/ "
            "views and git-commit them with MSG — one atomic edit->ingest->"
            "commit step (the rendered views are fresh, so the strategy-"
            "freshness gate passes). Incompatible with --dry-run."
        ),
    )
    parser.add_argument(
    ...
    if parsed is None:
        return 2
    if parsed.commit_message is not None and parsed.dry_run:
        return usage_error("--commit cannot be combined with --dry-run")
    try:
    ...
    _write_returned_files(target_root, response)
    ...
    if rc == 0 and parsed.commit_message is not None:
        from yoke_cli.commands.adapters.strategy_commit import (
            commit_written_views,
        )

        rc = commit_written_views(
            target_root, response, parsed.commit_message,
        )
    return rc
...
def _write_returned_files(target_root, response) -> None:
    ...
    if not entries:
        return
    write_rendered_files(target_root, entries)
...
def strategy_render(args: List[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Write the project's tracked .yoke/strategy/ rendered view "
            "from the DB authority (idempotent headers; unchanged content "
            "renders byte-identical). target_root resolves client-side: "
