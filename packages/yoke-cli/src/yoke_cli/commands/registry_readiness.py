...
from yoke_cli.commands.adapters.claims_path_flow import claims_path_boundary_prove
...
READINESS_SUBCOMMAND_REGISTRY: Dict[Tuple[str, ...], Tuple[str, AdapterFn]] = {
    ("readiness", "check"): ("readiness.check.run", _adapters.readiness_check),
    ("readiness", "prd-validate"): (
        "readiness.prd_validate.run",
        _adapters.readiness_prd_validate,
    ),
    ...
    ("claims", "path", "required-gate"): (
        ...
        _adapters.claims_path_required_gate,
    ),
    ("claims", "path", "activation-run"): (
        "claims.path.activation_run",
        _adapters.claims_path_activation_run,
    ),
    ("claims", "path", "boundary-prove"): (
        "claims.path.boundary_prove",
        claims_path_boundary_prove,
    ),
    ("claims", "path", "override"): ("claims.path.override", claims_path_override),
}
