...
contract (:mod:`yoke_core.domain.repo_upstream_freshness`), which the install
...
contract's :mod:`yoke_core.domain.repo_upstream_git` sibling rather than a
...
def network_git(
    ...
    Delegates to the shared upstream git runner so there is one place an
    engine builds a credentialed git invocation, and supplies only what is
    publication's own: the operator-configurable timeout, rather than that
    module's fixed read and fetch defaults.
    """
