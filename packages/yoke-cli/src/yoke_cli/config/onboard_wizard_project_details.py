...
def fields(*, slug: str, branch_from_source: str | None) -> tuple[_FormField, ...]:
    items = [
        _FormField(
            ...
            "Project ID",
            "my-project",
            validate=validation.validate_slug,
            initial_value=slug,
        ),
        _FormField(
            "name",
            "Display name",
            "My Project",
            validate=validation.validate_display_name,
            initial_value=slug,
        ),
    ]
    if not branch_from_source:
        items.append(
            _FormField(
                "branch",
                "Default branch",
                onboard_project.DEFAULT_NEW_REPO_BRANCH,
    ...
    items.append(
        _FormField(
            ...
            "Item prefix",
            "PROJ",
            validate=validation.validate_prefix,
...
def body(
    form_fields: tuple[_FormField, ...], *, branch_from_source: str | None
) -> list[Static]:
    ...
    widgets.append(
        SelectionList(
            [
                SelectionRow(
                    "continue", "Continue", "validate and save these project details"
                ),
            ]
        )
    )
