...
def generate_inventory(
    *, repo_root: Path | str | None = None, package_root: Path | str | None = None
) -> tuple[InventoryRow, ...]:
    ...
    for tokens, (function_id, adapter) in sorted(
        registry_items, key=lambda i: (" ".join(i[0]), i[1][0])
    ):
        shell = _shell(tokens)
        ...
        rows.append(
            _make_row(
                ...
                edges.get(source, ()),
                op,
                _owner(shell, op, adapter.__module__),
            )
        )
        seen.add(shell)
    for tokens, adapter in sorted(
        TOOL_SHAPED_SUBCOMMANDS.items(), key=lambda i: " ".join(i[0])
    ):
        shell = _shell(tokens)
        source = module_source(adapter.__module__, pkg, root)
        represented.add(source)
    ...
    return tuple(
        sorted(
            rows,
            key=lambda r: (
                order.get(r.disposition, len(order)),
                r.command_helper,
                r.function_id or "",
            ),
        )
...
def _make_row(
    ...
    operation: ops.OperationEntry | None,
    owner: str,
) -> InventoryRow:
...
def _branch(
    command: str, disposition: str, operation: ops.OperationEntry | None
) -> str:
    ...
    return (
        "client-local-tool"
        if operation and operation.reason == ops.REASON_TOOL_SHAPED
        else "client-local-helper"
    )
...
def _config(command: str, disposition: str) -> str:
    ...
    if command in by_command:
        return by_command[command]  # noqa: E701
    return {
...
def _capability(command: str, disposition: str) -> str:
    ...
    if command == "yoke runner-fleet exec":
        return "project aws-admin plus repository-bound GitHub App authority"  # noqa: E701
...
def _owner(shell_form: str, operation: ops.OperationEntry | None, module: str) -> str:
    return (
        ...
        if shell_form.startswith("yoke ")
        else "source-dev/admin"
    )
