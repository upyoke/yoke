...
def report_unregistered_launch_deaths(
    ...
    *,
    ...
    poll would say the same thing forever. A record whose report did not land
    is kept — a refused batch and one this poll had no room for alike — so the
    ...
    if not deaths:
        return ()
    reported: list[str] = []
    for batch, _result in deliver_liveness_batches(
        dispatcher,
        inventory,
        collection="launches",
        reports=[
        ...
        timeout_s=timeout_s,
        refusal_label="relay launch-death report",
    ):
        for report in batch:
