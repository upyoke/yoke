...
def test_onboard_local_checkout_dry_run_previews_project_handoff(
    ...
    assert payload["project_onboarding"]["github_adoption"]["choice"] == "disabled"
    assert "/yoke onboard --run-id" in payload["next_steps"][1]
...
def test_onboard_create_repo_dry_run_uses_project_create_branch(
    ...
    assert any(
        step["action"] == "project-create-checkout" for step in payload["plan"]["steps"]
    )
