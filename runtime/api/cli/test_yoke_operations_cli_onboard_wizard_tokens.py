...
def test_empty_token_prompt_stays_on_input() -> None:
    ...
    async def scenario() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.pause()
            assert app.query_one("#onboard-input-credential", Input).password is True
            error = app.query_one("#onboard-input-error-credential", Static)
...
def test_yoke_token_prompt_verifies_before_github() -> None:
    ...
    async def scenario() -> None:
        async with app.run_test() as pilot:
            ...
            assert "Yoke token: test-actor" in text
            assert "1 organizations" in text
...
def test_stored_yoke_token_shows_reuse_feedback(tmp_path) -> None:
    ...
    async def scenario() -> None:
        async with app.run_test() as pilot:
            await advance_past_path(pilot)
            text = await _wait_for_body_text(
                app, pilot, "Use this saved Yoke connection?"
            )
...
def test_yoke_token_prompt_error_can_retry(monkeypatch) -> None:
    ...
    async def scenario() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            # The destination replaces the failed form while retaining the
            # install step as the only user-visible Back target.
            assert len(app._history) == error_depth
            assert app.result.token is None
