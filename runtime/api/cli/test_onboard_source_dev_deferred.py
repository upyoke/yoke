"""Deferred editable install for "Edit Yoke source".
...
def test_source_dev_project_mode_needs_only_checkout() -> None:
    parsed = argparse.Namespace(
        project_mode=onboard_config.PROJECT_MODE_EDIT_YOKE_SOURCE,
...
def test_finish_pending_dev_install_uses_selected_stream(
    ...
    assert "Finalizing Yoke source activation" in output
    assert "`yoke` now runs from" in output
...
def test_run_editable_install_step_ok(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _root, **_kwargs: {"ran": True},
    )
    assert dev_setup.run_editable_install_step(root) == {
        "ok": True,
        "editable_install": {"ran": True},
    }
...
def test_run_editable_install_step_captures_failure(
    ...
    monkeypatch.setattr(activation, "_run_editable_install", _boom)
