...
def test_registry_operation_and_tool_shaped_surfaces_are_present():
    ...
    assert rows["yoke check file-line"].disposition == (inventory.CLIENT_LOCAL_HELPER)
    ...
    assert rows["yoke qa case run"].disposition == inventory.CLIENT_LOCAL_HELPER
    assert rows["yoke qa case run"].import_edges == ()
    assert "yoke qa browser run" not in rows
    ...
    assert (
        rows["python3 -m yoke_core.cli.db_router query"].disposition
        == inventory.OPERATOR_DEBUG_PERMANENT
    )
    assert (
        rows["yoke project-structure deploy-defaults get"].function_id
