"""CLI and inventory contracts for steering-scope work claims."""

from __future__ import annotations

import io
import json
from contextlib import redirect_stderr, redirect_stdout
from unittest.mock import patch

import pytest

from yoke_cli.main import main as cli_main
from yoke_contracts.api.function_call import FunctionCallRequest, FunctionCallResponse


_CAPTURED: list[FunctionCallRequest] = []


def _claim(
    claim_id: int,
    *,
    slugs: list[str],
    holder: str = "steering-session",
    released_at: str | None = None,
) -> dict:
    return {
        "id": claim_id,
        "session_id": holder,
        "target_kind": "steering_scope",
        "steering_project_id": 7,
        "steering_strategy_doc_slugs": slugs,
        "claim_type": "exclusive",
        "claimed_at": "2026-08-25T17:00:00Z",
        "last_heartbeat": "2026-08-25T17:01:00Z",
        "released_at": released_at,
        "release_reason": "completed" if released_at else None,
        "owner_kind": "session",
        "owner_item_id": None,
        "owner_session_id": holder,
        "owner_work_claim_id": None,
        "registered_by_actor_id": 2,
        "registered_by_session_id": holder,
    }


def _stub_response(request: FunctionCallRequest) -> FunctionCallResponse:
    _CAPTURED.append(request)
    if request.function == "claims.steering_scope.list":
        result = {
            "claims": [
                _claim(41, slugs=[]),
                _claim(42, slugs=["MISSION", "VISION"], holder="other-session"),
            ]
        }
    else:
        result = {"claim": _claim(41, slugs=[])}
    return FunctionCallResponse(
        success=True,
        function=request.function,
        version=request.version,
        request_id=request.request_id,
        result=result,
    )


@pytest.fixture(autouse=True)
def _reset_captured() -> None:
    _CAPTURED.clear()


def _run(*argv: str):
    with patch.dict("os.environ", {"YOKE_SESSION_ID": "steering-session"}):
        with (
            patch(
                "yoke_core.domain.yoke_function_dispatch.dispatch",
                side_effect=_stub_response,
            ),
            patch("yoke_cli.commands._helpers.ensure_handlers_loaded"),
        ):
            stdout, stderr = io.StringIO(), io.StringIO()
            with redirect_stdout(stdout), redirect_stderr(stderr):
                rc = cli_main(list(argv))
            return rc, stdout.getvalue(), stderr.getvalue()


def test_acquire_dispatches_project_target_and_repeated_doc_scope() -> None:
    rc, _out, err = _run(
        "claims",
        "steering-scope",
        "acquire",
        "--project",
        "alpha",
        "--strategy-doc",
        "MISSION",
        "--strategy-doc",
        "VISION",
        "--reason",
        "guide planning",
    )
    assert rc == 0, err
    request = _CAPTURED[-1]
    assert request.function == "claims.steering_scope.acquire"
    assert request.target.kind == "global"
    assert request.target.project_id == "alpha"
    assert request.payload == {
        "strategy_doc_slugs": ["MISSION", "VISION"],
        "reason": "guide planning",
    }


def test_acquire_omitted_doc_scope_means_whole_project() -> None:
    rc, out, err = _run(
        "claims",
        "steering-scope",
        "acquire",
        "--project",
        "alpha",
    )
    assert rc == 0, err
    assert _CAPTURED[-1].payload == {"strategy_doc_slugs": []}
    assert "scope=whole-project" in out
    assert "holder=steering-session" in out


def test_release_dispatches_claim_target_and_reason() -> None:
    rc, out, err = _run(
        "claims",
        "steering-scope",
        "release",
        "41",
        "--reason",
        "steering complete",
    )
    assert rc == 0, err
    request = _CAPTURED[-1]
    assert request.function == "claims.steering_scope.release"
    assert request.target.kind == "claim"
    assert request.target.claim_id == 41
    assert request.payload == {"reason": "steering complete"}
    assert "released steering-scope claim 41" in out


def test_release_rejects_non_integer_claim_id() -> None:
    rc, _out, err = _run(
        "claims",
        "steering-scope",
        "release",
        "nope",
        "--reason",
        "done",
    )
    assert rc == 2
    assert "CLAIM_ID must be an integer" in err
    assert _CAPTURED == []


def test_list_dispatches_project_and_filters() -> None:
    rc, _out, err = _run(
        "claims",
        "steering-scope",
        "list",
        "--project",
        "alpha",
        "--session-id",
        "other-session",
        "--active-only",
    )
    assert rc == 0, err
    request = _CAPTURED[-1]
    assert request.function == "claims.steering_scope.list"
    assert request.target.kind == "global"
    assert request.target.project_id == "alpha"
    assert request.payload == {
        "session_id": "other-session",
        "active_only": True,
    }


def test_human_list_names_whole_project_doc_set_and_holders() -> None:
    rc, out, err = _run(
        "claims",
        "steering-scope",
        "list",
        "--project",
        "alpha",
    )
    assert rc == 0, err
    assert "claim_id\tproject\tscope\tholder\tstate" in out
    assert "41\t7\twhole-project\tsteering-session\tactive" in out
    assert "42\t7\tMISSION,VISION\tother-session\tactive" in out


def test_json_list_emits_response_envelope() -> None:
    rc, out, err = _run(
        "claims",
        "steering-scope",
        "list",
        "--project",
        "alpha",
        "--json",
    )
    assert rc == 0, err
    payload = json.loads(out)
    assert payload["success"] is True
    assert payload["function"] == "claims.steering_scope.list"
    assert payload["result"]["claims"][0]["steering_strategy_doc_slugs"] == []


@pytest.mark.parametrize("operation", ("acquire", "release", "list"))
def test_help_exposes_steering_scope_grammar(operation: str) -> None:
    with patch("yoke_cli.commands._helpers.ensure_handlers_loaded"):
        stdout, stderr = io.StringIO(), io.StringIO()
        with redirect_stdout(stdout), redirect_stderr(stderr):
            rc = cli_main(["claims", "steering-scope", operation, "--help"])
    assert rc == 0
    assert f"yoke claims steering-scope {operation}" in stdout.getvalue()


def test_registry_inventory_and_usage_expose_all_operations() -> None:
    from yoke_cli.commands.adapters.usage import ADAPTER_USAGE
    from yoke_cli.commands.registry import SUBCOMMAND_REGISTRY
    from yoke_core.api.service_client_structured_api_adapter_inventory import (
        adapter_index,
    )

    expected = {
        "acquire": "claims.steering_scope.acquire",
        "release": "claims.steering_scope.release",
        "list": "claims.steering_scope.list",
    }
    inventory = adapter_index()
    for operation, function_id in expected.items():
        assert (
            SUBCOMMAND_REGISTRY[("claims", "steering-scope", operation)][0]
            == function_id
        )
        assert function_id in ADAPTER_USAGE
        assert function_id in inventory


def test_function_registry_exposes_steering_scope_contracts() -> None:
    from yoke_core.domain.handlers.__init_register__ import register_all_handlers
    from yoke_core.domain.yoke_function_registry import lookup

    register_all_handlers()
    acquire = lookup("claims.steering_scope.acquire")
    release = lookup("claims.steering_scope.release")
    listed = lookup("claims.steering_scope.list")
    assert acquire is not None and acquire.claim_required_kind is None
    assert acquire.target_kinds == ("global",)
    assert acquire.emitted_event_names == ("SteeringScopeClaimed",)
    assert release is not None and release.claim_required_kind == "self_only"
    assert release.target_kinds == ("claim",)
    assert listed is not None and listed.side_effects == ()
