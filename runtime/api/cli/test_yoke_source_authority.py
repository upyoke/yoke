...
def test_begin_requires_attended_service_stop_receipt(capsys):
    assert source_authority.source_authority_quiesce(["begin"]) == 2
...
def test_begin_dispatches_client_local_admin_surface(monkeypatch, capsys):
    ...
    assert rc == 0
    assert seen == {"service_stop_receipt": "stopped-123"}
...
def test_database_failures_return_safe_operator_errors(monkeypatch, capsys):
    def fail(**_kwargs):
        raise psycopg.OperationalError(
            "connection failed for password=do-not-echo host=private-db"
        )
    ...
    assert source_authority.source_authority_quiesce(["status"]) == 1
