...
class TestDbReadDispatch:
    ...
    def test_db_read_lines_format_emits_shell_consumable_rows(self) -> None:
        def stub(request: FunctionCallRequest) -> FunctionCallResponse:
            _CAPTURED_REQUESTS.append(request)
            return FunctionCallResponse(
                success=True,
        ...
        rc, out, err = _run_capture(
            stub, "db", "read", "SELECT id, title FROM items",
            "--format", "lines",
        )
    ...
    def test_db_read_json_envelope_rejects_lines_format(self) -> None:
        rc, _out, err = _run_capture(
            _stub_dispatch_ok, "db", "read", "SELECT 1",
            "--format", "lines", "--json",
        )
