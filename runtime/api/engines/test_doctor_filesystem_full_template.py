"""Doctor filesystem HC tests: project templates, schema sync, body, config.
...
class TestProjectTemplateAndSchemaChecks:
    def test_template_project_drift_passes_without_repo_root(self):
        with patch("yoke_core.engines.doctor_report._resolve_repo_root", return_value=None):
            rec = _run_hc(hc_template_project_drift)
        assert rec.results[0].result == "PASS"

    def test_template_project_drift_passes_when_layout_missing(self, tmp_path):
    ...
    def test_template_project_drift_check_is_retired(self, tmp_path):
        ...
        assert rec.results[0].result == "PASS"
        assert "no longer tracked" in rec.results[0].detail
    ...
    def test_template_project_drift_passes_on_matching_sizes(self, tmp_path):
        ...
        (templates_dir / "deploy.sh.tmpl").write_text(content)
        (project_ops_dir / "deploy.sh").write_text(content)
        with patch("yoke_core.engines.doctor_report._resolve_repo_root", return_value=str(tmp_path)):
            rec = _run_hc(hc_template_project_drift, conn)
        assert rec.results[0].result == "PASS"
