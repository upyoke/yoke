...
A ``disabled`` project keeps its backlog in the Yoke DB only: after
the documented sync-off -> repo-flip cutover its ``github_issue`` refs
stay historical (.yoke/docs/reference/github-sync.md). The wrong-repo and GitHub-orphan
...
def _set_disabled(conn, slug: str) -> None:
    conn.execute("ALTER TABLE projects ADD COLUMN IF NOT EXISTS github_sync_mode TEXT")
    conn.execute(
        "UPDATE projects SET github_sync_mode = 'disabled' WHERE slug = %s",
        (slug,),
    )
    conn.commit()
...
class TestGhOrphanDetectionSyncMode:
    ...
    def test_disabled_repo_not_searched(
        self,
        mock_rest,
        mock_resolve,
        mock_avail,
    ):
        """A disabled project's repo is never searched for [YOK-]
