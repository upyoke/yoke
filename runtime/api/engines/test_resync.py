"""Tests for resync engine helpers and stage-1 fetch behavior.

Repair tests live in test_resync_repair.py.
Stage-2 compare and normalize tests live in test_resync_compare.py.
GraphQL batch transport tests live in test_resync_graphql_batch.py.
...
class TestHelpers:
    ...
    def test_resolve_yoke_root_uses_python_path_helper(self, monkeypatch):
        ...
        with mock.patch(
            "yoke_core.engines.resync.resolve_worktree_yoke_root",
            return_value="/tmp/resolved-root",
        ):
    ...
    def test_stage1_linkage_builds_pairs_local_orphans_and_gh_orphans(self, populated_db, tmp_path):
        ...
        gh_map = {
            "yoke": {
                ...
                200: {
                    "number": 200,
                    "title": "[YOK-1246] Task 001 Task one",
                    "labels": [],
                    "state": "OPEN",
                    "body": "",
                },
