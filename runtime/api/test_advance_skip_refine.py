...
class TestSkipRefineHappyPath:
    def test_idea_to_refined_idea_issue(self):
        ...
        patches = _patch_core("idea", "issue", update_recorder=exec_recorder)
        _enter_all(patches)
        try:
    ...
    def test_refining_idea_to_refined_idea_issue(self):
        exec_recorder = _CallRecorder()
        patches = _patch_core("refining-idea", "issue", update_recorder=exec_recorder)
        _enter_all(patches)
        try:
    ...
    def test_refining_idea_to_refined_idea_epic(self):
        exec_recorder = _CallRecorder()
        patches = _patch_core("refining-idea", "epic", update_recorder=exec_recorder)
        _enter_all(patches)
        try:
    ...
    def test_refining_plan_to_planned_epic(self):
        exec_recorder = _CallRecorder()
        patches = _patch_core("refining-plan", "epic", update_recorder=exec_recorder)
        _enter_all(patches)
        try:
    ...
    def test_plan_drafted_to_planned_epic(self):
        exec_recorder = _CallRecorder()
        patches = _patch_core("plan-drafted", "epic", update_recorder=exec_recorder)
        _enter_all(patches)
        try:
            result = advance_skip.skip_refine(202, out=io.StringIO())
        finally:
...
class TestSkipRefineRejection:
    ...
    def test_refining_plan_rejected_when_workflow_omits_target(self):
        ...
        _enter_all(patches)
        try:
            with pytest.raises(ValueError, match="not declared by issue@"):
