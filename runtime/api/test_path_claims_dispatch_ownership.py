...
from yoke_core.domain import path_claims_dispatch, path_claims_dispatch_amend, path_claims_dispatch_narrow, path_claims_dispatch_state
...
def _seed_path_claim(conn, *, actor_id, item_id=ITEM_ID, target_ids=()):
    ...
    for tid in target_ids:
        conn.execute("INSERT INTO path_claim_targets (claim_id, target_id, declared_at) VALUES (%s, %s, '2026-05-01T00:00:00Z')", (claim_id, tid))
    return claim_id


def _projection_snapshot(conn, claim_id):
    pc = conn.execute("SELECT state, activated_at, released_at, cancelled_at, blocked_reason FROM path_claims WHERE id = %s", (claim_id,)).fetchone()
    targets = sorted(
        r[0]
        for r in conn.execute(
            "SELECT pt.path_string FROM path_claim_targets pct JOIN path_targets pt ON pt.id = pct.target_id WHERE pct.claim_id = %s", (claim_id,)
        ).fetchall()
    )
    amend_count = conn.execute("SELECT COUNT(*) FROM path_claim_amendments WHERE claim_id = %s", (claim_id,)).fetchone()[0]
...
def patch_conn(monkeypatch, conn):
    ...
    monkeypatch.setattr(path_claims_dispatch, "boundary_check_for_claim", lambda *a, **kw: ok())
...
def staged(patch_conn):
    ...
    claim_id = _seed_path_claim(patch_conn, actor_id=actor, target_ids=(foo, bar))
    patch_conn.commit()
...
class TestRegister:
    def test_register_denied_for_non_holder(self, staged, monkeypatch, capsys):
        ...
        before_targets = conn.execute("SELECT COUNT(*) FROM path_claim_targets").fetchone()[0]

        rc = path_claims_dispatch.cmd_register(
            ["--item", f"YOK-{ITEM_ID}", "--integration-target", "main", "--paths", "src/foo.py", "--actor-id", str(staged["actor"])]
        )
        ...
        assert conn.execute("SELECT COUNT(*) FROM path_claims").fetchone()[0] == before_claims
        assert conn.execute("SELECT COUNT(*) FROM path_claim_targets").fetchone()[0] == before_targets

    def test_register_session_id_flag_does_not_bypass_guard(self, staged, monkeypatch, capsys):
        monkeypatch.setenv("YOKE_SESSION_ID", INTRUDER_SESSION)
        rc = path_claims_dispatch.cmd_register(
            [
                "--item",
    ...
    def test_register_denied_when_no_work_claim_exists(self, patch_conn, monkeypatch, capsys):
        actor = local_human(patch_conn)
        ...
        patch_conn.commit()
        monkeypatch.setenv("YOKE_SESSION_ID", INTRUDER_SESSION)
        rc = path_claims_dispatch.cmd_register(["--item", "YOK-9002", "--integration-target", "main", "--paths", "src/foo.py", "--actor-id", str(actor)])
        payload = _err_payload(capsys)
        assert rc == 1
        assert payload["code"] == "OWNERSHIP_DENIED"
...
def _activate(cid):
    return path_claims_dispatch_state.cmd_activate([str(cid), "--base-commit-sha", "deadbeef0001"])
...
def _widen(cid):
    return path_claims_dispatch_amend.cmd_widen([str(cid), "--paths", "src/bar.py", "--reason", "intrude"])


def _cancel_amendment(cid):
    return path_claims_dispatch_amend.cmd_cancel_amendment([str(cid), "--amendment-id", "9999", "--reason", "intrude"])
...
class TestStateAndAmendmentsDeniedForNonHolder:
    ...
    @pytest.mark.parametrize(
        "call", [_activate, _release, _cancel, _widen, _cancel_amendment], ids=["activate", "release", "cancel", "widen", "cancel_amendment"]
    )
    def test_mutation_denied_and_state_unchanged(self, staged, capsys, call):
        ...
        assert _projection_snapshot(staged["conn"], staged["claim_id"]) == before

    def test_narrow_denied_and_state_unchanged(self, staged, tmp_path, capsys):
        before = _projection_snapshot(staged["conn"], staged["claim_id"])
        rc = path_claims_dispatch_narrow.cmd_narrow(
            [str(staged["claim_id"]), "--drop-paths", "src/bar.py", "--reason", "intrude", "--repo-path", str(tmp_path)]
        )
        payload = _err_payload(capsys)
        assert rc == 1
        assert payload["code"] == "OWNERSHIP_DENIED"
...
class TestReadOnlyRemainsAllowedForNonHolder:
    ...
    @pytest.mark.parametrize("call", [_get, _list_for_item, _conflicts, _boundary], ids=["get", "list", "conflicts", "boundary"])
...
class TestServiceClientForwardingObservesGuard:
    def test_path_claim_widen_via_service_client_denied(self, staged, monkeypatch, capsys):
        monkeypatch.setenv("YOKE_SESSION_ID", INTRUDER_SESSION)
        before = _projection_snapshot(staged["conn"], staged["claim_id"])
        rc = cmd_path_claim_widen([str(staged["claim_id"]), "--paths", "src/bar.py", "--reason", "intrude-via-service-client"])
        payload = _err_payload(capsys)
        assert rc == 1
        assert payload["code"] == "OWNERSHIP_DENIED"
        assert payload["claim_id"] == staged["claim_id"]
