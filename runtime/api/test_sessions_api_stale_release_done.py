...
class TestReleaseClaimsForDoneItem:
    ...
    def test_releases_unreleased_claims_on_done_item(self, conn):
        ...
        claim = claim_work(conn, session_id="stale-sess", item_id=9999)

        released = release_claims_for_done_item(conn, 9999)
    ...
    def test_no_claims_returns_zero(self, conn):
        ...
        released = release_claims_for_done_item(conn, 999)
        assert released == 0
    ...
    def test_skips_already_released_claims(self, conn):
        ...
        claim = claim_work(conn, session_id="old-sess", item_id=9999)
    ...
    def test_emits_per_claim_telemetry(self, mock_emit, conn):
        ...
        _register(conn, session_id="stale-sess")
        claim_work(conn, session_id="stale-sess", item_id=9999)
    ...
    def test_regression_claim_row_residue(self, conn):
        ...
        released = release_claims_for_done_item(conn, 1187)
        assert released == 1
        row = conn.execute(
