...
class TestRenderPulumiArtifacts:
    ...
    def test_writes_seven_files(self, infra_tree, monkeypatch):
        ...
        values = {
            ...
            "pulumi_vps_stack_name": "externalwebapp-vps",
        }
        ...
        expected = {
            "Pulumi.yaml",
            "Pulumi.externalwebapp-infra.yaml",
            "Pulumi.externalwebapp-vps.yaml",
            "__main__.py",
        ...
        infra_stack = (infra_dst / "Pulumi.externalwebapp-infra.yaml").read_text()
        assert "us-east-1" in infra_stack
        assert "externalwebapp" in infra_stack
        assert "{{" not in infra_stack

        vps_stack = (infra_dst / "Pulumi.externalwebapp-vps.yaml").read_text()
        ...
        assert not (infra_dst / "externalwebapp_infra_stack.py").exists()
        assert not (infra_dst / "externalwebapp_vps_stack.py").exists()
    ...
    def test_preserves_operator_secrets_provider(self, infra_tree, monkeypatch):
        ...
        infra_stack_path = proj / "infra" / "Pulumi.externalwebapp-infra.yaml"
        first_render = infra_stack_path.read_text()
        assert "secretsprovider:" not in first_render

        operator_state = (
            "secretsprovider: awskms://alias/externalwebapp-pulumi-state?region=us-east-1\n"
        ...
        assert "awskms://alias/externalwebapp-pulumi-state?region=us-east-1" in re_rendered
...
_PROVIDER = "awskms://alias/externalwebapp-pulumi-state?region=us-east-1"
...
class TestOperatorStateLinesFromSettings:
    ...
    def test_matching_stack_renders_both_lines(self):
        settings = _settings_with_pulumi_state(
            "externalwebapp", "externalwebapp-prod",
            secrets_provider=_PROVIDER, encrypted_key=_KEY,
        )
        assert _operator_state_lines_from_settings(settings, "externalwebapp-prod") == (
    ...
    def test_non_matching_stack_renders_nothing(self):
        ...
        assert _operator_state_lines_from_settings(settings, "externalwebapp-infra") == ""

    def test_absent_keys_render_nothing(self):
        settings = _settings_with_pulumi_state("externalwebapp", "externalwebapp-prod")
        assert _operator_state_lines_from_settings(settings, "externalwebapp-prod") == ""

    def test_single_present_key_renders_alone(self):
        settings = _settings_with_pulumi_state(
            "externalwebapp", "externalwebapp-prod", secrets_provider=_PROVIDER,
        )
        assert _operator_state_lines_from_settings(settings, "externalwebapp-prod") == (
            f"secretsprovider: {_PROVIDER}\n"
        )
...
class TestFreshRenderSettingsFallback:
    ...
    def test_fresh_render_sources_state_lines_from_settings(
        ...
        self._stub(monkeypatch, _settings_with_pulumi_state(
            "externalwebapp", "externalwebapp-infra",
            secrets_provider=_PROVIDER, encrypted_key=_KEY,
        ))
        project_renderer_pulumi.render_pulumi_artifacts(
            "externalwebapp", dict(self._VALUES), root, proj, write=True,
        )
        rendered = (proj / "infra" / "Pulumi.externalwebapp-infra.yaml").read_text()
    ...
    def test_fresh_render_without_settings_keys_is_unchanged(
        ...
        root, proj = infra_tree
        self._stub(
            monkeypatch, _settings_with_pulumi_state("externalwebapp", "externalwebapp-infra"),
        )
        project_renderer_pulumi.render_pulumi_artifacts(
            "externalwebapp", dict(self._VALUES), root, proj, write=True,
        )
    ...
    def test_existing_file_lines_win_over_settings(
        ...
        out_path = proj / "infra" / "Pulumi.externalwebapp-infra.yaml"
