...
class TestEnvDeployWorkflowShape:
    """The CI lane selects wait mode explicitly + carries a dispatch lane."""
    ...
    def workflow_text(self) -> str:
        ...
        path = (
            ...
            / ".github" / "workflows" / "yoke-env-deploy.yml"
        )
        return path.read_text()

    def test_dispatch_retrigger_lane_present(self, workflow_text):
        assert "workflow_dispatch:" in workflow_text
        # The push lane stays — dispatch is additive, not a replacement.
        assert "branches: [stage]" in workflow_text

    def test_wait_mode_selected_explicitly(self, workflow_text):
        assert f'{IMAGE_WAIT_ENV_VAR}: "1"' in workflow_text

    def test_prewarm_gate_waits_before_skip(self, workflow_text):
        assert 'YOKE_IMAGE_PREWARM_WAIT_SECONDS: "900"' in workflow_text
        assert 'YOKE_IMAGE_PREWARM_POLL_SECONDS: "15"' in workflow_text
        assert "deadline=$((SECONDS + wait_seconds))" in workflow_text
        assert "waiting for prewarmed image yoke-core:$deploy_image_tag" in workflow_text
        assert "within ${wait_seconds}s" in workflow_text

    def test_distribution_publish_waits_for_release_flow(self, workflow_text):
        assert "deployed: ${{ steps.release_flow.outputs.deployed }}" in workflow_text
        assert "id: release_flow" in workflow_text
        assert 'echo "deployed=true" >> "$GITHUB_OUTPUT"' in workflow_text
        assert "publish-stage-distribution:" in workflow_text
        assert "needs.env-deploy.outputs.deployed == 'true'" in workflow_text

    def test_deploy_step_uses_checkout_git_fetch_auth_once(self, workflow_text):
        assert "persist-credentials: true" in workflow_text
        assert "GIT_CONFIG_COUNT" not in workflow_text
        assert "http.https://github.com/.extraheader" not in workflow_text
