...
def test_dependency_add_accepts_coordination_only(conn: Any) -> None:
    ...
    rows = conn.execute(
        "SELECT dependent_item_id, blocking_item_id, gate_point, satisfaction "
        "FROM item_dependencies"
    ).fetchall()
    stored = [(int(row[0]), int(row[1]), row[2], row[3]) for row in rows]
    assert stored == [(1, 2, "coordination_only", "fact:merged")]
...
def test_evaluate_batch_gates_activation_ignores_coordination_only() -> None:
    ...
    insert_dep(db, "YOK-1", "YOK-3", gate_point="coordination_only")
    db.commit()
...
def test_evaluate_item_gate_activation_ignores_coordination_only() -> None:
    ...
    insert_item(db, 1, status="planned")
    insert_item(db, 2, status="implementing")
    insert_dep(db, "YOK-1", "YOK-2", gate_point="coordination_only")
    db.commit()
...
def test_frontier_unblocks_count_ignores_coordination_only() -> None:
    ...
    insert_dep(db, "YOK-2", "YOK-3", gate_point="coordination_only")
    db.commit()
