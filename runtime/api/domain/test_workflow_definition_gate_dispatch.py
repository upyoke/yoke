    ...
    *,
    ...
    try:
        insert_item(
            ...
            status="idea" if workflow_id == "dash" else "refined-idea",
        )
        now = iso8601_now()
        conn.execute(
...
def _run_implementing_gate(
    db_path: str, *, item_id: int, session_id: str
) -> dict | None:
...
def test_dash_activation_requires_the_live_claim_and_registered_worktree(
    direct_db_path: str, monkeypatch: pytest.MonkeyPatch
):
    ...
    _seed_activation(
        ...
        item_id=2170,
        workflow_id="dash",
        session_id="dash-activation",
        lane_role=LANE_IMPLEMENTATION,
        touch_path="src/dash_activation.py",
    )

    assert (
        _run_implementing_gate(
            direct_db_path, item_id=2170, session_id="dash-activation"
        )
        is None
    )
    ...
    try:
        conn.execute(
            "UPDATE item_worktrees SET state='released', released_at=%s WHERE item_id=2170 AND state='active'",
            (iso8601_now(),),
        )
        conn.commit()
    finally:
        conn.close()
    blocked = _run_implementing_gate(
        direct_db_path, item_id=2170, session_id="dash-activation"
    )
...
def test_blitz_lifecycle_claims_refuses_conflict_and_releases_terminally(
    direct_db_path: str, monkeypatch: pytest.MonkeyPatch
):
    from yoke_core.domain import backlog_architecture_gate_runner
    ...
    _seed_activation(
        ...
        item_id=2180,
        workflow_id="blitz",
        session_id="blitz-owner",
        lane_role=LANE_WORKER,
        touch_path="src/blitz_owner.py",
    )
    conn = connect_test_db(direct_db_path)
    try:
        create_doc(
    ...
    assert (
        _run_implementing_gate(direct_db_path, item_id=2180, session_id="blitz-owner")
        is None
    )
    conn = connect_test_db(direct_db_path)
    try:
    ...
    _seed_activation(
        ...
        lane_role=LANE_WORKER,
        touch_path="src/blitz_contender.py",
    )
    conn = connect_test_db(direct_db_path)
    try:
        link_execution_document(
        ...
        conn.close()
    conflict = _run_implementing_gate(
        direct_db_path, item_id=2181, session_id="blitz-contender"
    )
