...
class TestItemLessRun:
    ...
    def test_item_less_run_executes_to_success(self, capsys, monkeypatch):
        ...
        with mock.patch.object(
            ...
            deploy_pipeline.control_plane, "project_field", return_value="",
        ), mock.patch.object(
            deploy_pipeline.control_plane, "seed_qa", return_value=0,
        ), mock.patch.object(
            ...
            deploy_pipeline.control_plane, "record_qa_stage",
        ), mock.patch.object(
            deploy_pipeline.control_plane, "unresolved_qa", return_value=[],
        ):
...
class TestResolveAndVerifyBranch:
    ...
    def test_member_items_resolve_branch_and_verify(self, tmp_path):
        ...
        with mock.patch.object(
            ...
            ok, first_item, branch = (
                deploy_pipeline_gates._resolve_and_verify_branch(
                    ["42", "43"], str(tmp_path), target_branch="main",
                    first_branch="feature-x", sd=None,
                )
            )
    ...
    def test_failed_verification_propagates_not_ok(self, tmp_path, capsys):
        ...
        with mock.patch.object(
            ...
            ok, _first_item, _branch = (
                deploy_pipeline_gates._resolve_and_verify_branch(
                    ["42"], str(tmp_path), target_branch="main",
                    first_branch="feature-x", sd=None,
                )
            )
        assert ok is False
