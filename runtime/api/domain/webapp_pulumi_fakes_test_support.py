"""Fake ``pulumi`` / ``pulumi_aws`` modules for Pack stack tests.

The fake resource classes record constructor kwargs so tests can assert
the declared AWS surface without running the Pulumi engine. Shared by the
registry, environment, and runner-fleet stack tests, and kept out of any
one test file so none of them approaches the authored-file line limit.
"""
...
def _build_fake_aws(recorder):
    ...
    aws.autoscaling = types.SimpleNamespace(
        ...
        GroupMixedInstancesPolicyArgs=_FakeArgs,
        GroupMixedInstancesPolicyLaunchTemplateArgs=_FakeArgs,
        GroupMixedInstancesPolicyLaunchTemplateLaunchTemplateSpecificationArgs=(
            _FakeArgs
        ),
        GroupMixedInstancesPolicyInstancesDistributionArgs=_FakeArgs,
    )
