"""An attested project gets a blocking review where its command would have run."""
...
def test_an_attested_project_gets_one_blocking_review_requirement() -> None:
    ...
    assert len(rows) == 1
    assert rows[0]["qa_kind"] == NO_TESTS_REVIEW_QA_KIND
...
def test_an_unattested_project_seeds_nothing() -> None:
    with test_database() as conn:
        item = _seeded_item(conn)
