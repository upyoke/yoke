...
that resolves the checkout root by walking up from ``Path(__file__)`` to the
directory containing ``runtime/agents``.
...
def resolve_live_repo_root() -> Path:
    ...
    Walks up from this module's ``__file__`` to the directory that contains
    ``runtime/agents``. Raises ``RuntimeError`` when the checkout cannot be
    found, so a future packaging refactor does not silently fall back to
    ``git rev-parse``.
    """
    ...
    raise RuntimeError(
        ...
        "root; the runtime/agents walk-up anchor produced no hit"
    )
