...
def _seed_item(conn, *, item_id: int, project: str = "yoke") -> int:
    ...
    conn.execute(
        ...
        "VALUES (%s, 'item', 'issue', (SELECT current_version_id FROM workflows WHERE id='issue'), 'refining-idea', 'medium', "
...
class TestRepair:
    ...
    def test_repair_returns_evidence_packet(self, conn):
        ...
        rows = conn.execute(
            ...
            "WHERE dependent_item_id=%s OR blocking_item_id=%s",
            (_CANDIDATE_ITEM, _CANDIDATE_ITEM),
        ).fetchone()
        assert rows[0] == 0
