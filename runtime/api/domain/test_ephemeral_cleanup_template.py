...
class TestRenderedExternalWebappCleanup:
    ...
    def test_auto_generated_header_names_managed_refresh(self, rendered: str) -> None:
        """Rendered projects must use the packaged reconciliation boundary."""
        assert "AUTO-GENERATED from Yoke managed template" in rendered
        assert (
            "yoke project artifacts refresh --project externalwebapp --apply"
            in rendered
        )
        assert "yoke_core.tools.render_project" not in rendered
