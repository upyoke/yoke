...
def test_idle_heartbeat_uses_backoff_and_persists_public_facts_only() -> None:
    ...
    row = conn.execute(
        "SELECT relay_version,surface_versions,project_checkouts,connected_until "
        "FROM session_relays WHERE relay_id=?",
        (RELAY_ID,),
    ).fetchone()
    assert row[0] == "0.1.1"
    assert json.loads(row[1]) == {"codex-cli": "0.148.0a15"}
    assert json.loads(row[2]) == [10]
    assert row[3] == "2026-08-22T12:10:00Z"
    assert "relay-host" not in " ".join(str(value) for value in row)
...
def test_recent_hook_activity_snaps_idle_machine_to_active_cadence() -> None:
    ...
    outcome = claim_relay_job(
        conn, _heartbeat(), wait_seconds=0, now_provider=_clock()
    )
...
def test_live_lease_blocks_a_second_job_and_expires_without_guessing() -> None:
    ...
    assert settle_expired_relay_leases(
        conn, now="2026-08-22T12:01:31Z"
    ) == 1
...
def test_private_versions_fail_closed_outside_the_pinned_release() -> None:
    assert surface_operation_supported(
        "claude-cli", "2.1.238", "message_stopped"
    )
    assert not surface_operation_supported(
        "claude-cli", "2.1.239", "message_stopped"
    )
    assert not surface_operation_supported(
