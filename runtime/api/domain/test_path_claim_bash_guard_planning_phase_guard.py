...
pytest_plugins = ("runtime.api.domain.path_claim_bash_guard_planning_phase_fixture",)
...
def _planned_no_wt_claim(item_id=1844) -> Dict[str, Any]:
    return {
        ...
        "state": "planned",
        "covered_paths": ("runtime/api/domain/foo.py",),
        "worktree_path": "",
...
def _patch_guard(monkeypatch, claim):
    monkeypatch.setattr(
        ...
        lambda session_id, conn=None, **_kwargs: claim,
    )
...
def test_matrix_planning_session_code_edit_denies(tmp_path, widener_db, monkeypatch):
    ...
    v = bash_guard.evaluate_payload(
        _payload(
            command="rm runtime/api/domain/other.py", cwd=str(tmp_path), session_id=sid
        )
    )
...
def test_matrix_impl_session_scratch_outside_claim_denies(
    ...
    v = bash_guard.evaluate_payload(
        _payload(command=f"printf '%s' \"$b\" > {target}", cwd=str(wt), session_id=sid)
    )
    assert v.outcome == "deny"
...
def test_matrix_impl_session_code_edit_inside_claim_allows(
    ...
    monkeypatch.setenv("YOKE_SESSION_ID", sid)
    wt = tmp_path / "YOK-2024"
    (wt / "runtime/api/domain").mkdir(parents=True)
    _patch_guard(
        monkeypatch,
        {
            "id": 400,
    ...
    v = bash_guard.evaluate_payload(
        _payload(command="rm runtime/api/domain/bar.py", cwd=str(wt), session_id=sid)
    )
    assert v.outcome == "allow"
...
def test_original_worktree_unresolved_denial_no_longer_fires(
    ...
    v = bash_guard.evaluate_payload(
        _payload(command=cmd, cwd=str(tmp_path), session_id=sid)
    )
    assert v.outcome == "allow"
...
def test_retired_data_sessions_path_hits_original_denial(
    ...
    v = bash_guard.evaluate_payload(
        _payload(command=cmd, cwd=PROJECT_REPO_ROOT, session_id=sid)
    )
    assert v.outcome == "deny"
    assert v.failure_mode == "worktree-unresolved"
