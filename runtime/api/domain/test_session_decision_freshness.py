...
from yoke_core.domain.session_contract import ActionKind, ClaimedWork, FrontierState, SessionOffer
from yoke_core.domain.session_decision_charge import decide_charge_action
from yoke_core.domain.session_decision_freshness import FreshnessOutcome, evaluate_freshness
...
def _make_offer(*, session_id=_SESSION_ID, supported_paths=None, step=1, execution_lane="DARIUS") -> SessionOffer:
    return SessionOffer(
...
def _chain_skip_memory(conn, session_id: str = _SESSION_ID) -> list:
    row = conn.execute("SELECT offer_envelope FROM harness_sessions WHERE session_id = %s", (session_id,)).fetchone()
...
def _event_count(conn, *, event_name: str, session_id: str = _SESSION_ID) -> int:
    return sum(1 for e in _EMITTED_EVENTS if e["event_name"] == event_name and e.get("session_id") == session_id)


def _active_claim_count(conn, item_id: int) -> int:
    return conn.execute(
        "SELECT COUNT(*) FROM work_claims WHERE target_kind='item' AND scope = %s AND released_at IS NULL", (make_item_target(item_id).scope_json(),)
    ).fetchone()[0]
...
def _eval(
    ...
    *,
    ...
    return evaluate_freshness(
        ...
        scheduler_context=scheduler_context or {"status": expected_status, "next_step": expected_next_step},
...
def _charge_frontier(item_id, *, status="refined-idea", next_step="advance"):
    return FrontierState(
        ...
        scheduler_context={"status": status, "next_step": next_step, "workflow_id": "issue"},
        sml_coherent=True,
    )
...
def _capture_emitted_events(monkeypatch):
    ...
    monkeypatch.setattr(_events_mod, "emit_event", lambda event_name, **kw: _EMITTED_EVENTS.append({"event_name": event_name, **kw}))
...
class TestEvaluateFreshness:
    ...
    def test_failopen_when_conn_unavailable(self, monkeypatch, test_db):
        ...
        monkeypatch.setattr(fresh_mod, "connect", lambda: (_ for _ in ()).throw(FileNotFoundError("no DB")))
...
class TestChargeDispatchContextGuard:
    def test_missing_next_step_returns_wait(self):
        ...
        frontier = FrontierState(
            ...
            scheduler_context={"selected_item": "YOK-5001", "skipped_process": {"reason": "STRATEGIZE recommended but disabled"}},
            sml_coherent=True,
        )
        result = decide_charge_action(offer, frontier, offer.session_id, None)
        assert result.action is ActionKind.WAIT
    ...
    def test_no_scheduler_context_falls_through_to_fallback_branch(self):
        offer = _make_offer()
        frontier = FrontierState(runnable_items=["YOK-5004"], selected_item="YOK-5004", scheduler_context=None, sml_coherent=True)
        result = decide_charge_action(offer, frontier, offer.session_id, None)
        assert result.action is ActionKind.CHARGE
