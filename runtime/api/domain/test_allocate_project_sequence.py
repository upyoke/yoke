...
refilling low gaps, so externalwebapp (whose backfilled band starts at 662) never hands a
...
def test_empty_project_allocates_one(seeded):
    assert allocate_project_sequence(seeded, EXT_PROJECT_ID) == 1


def test_continues_from_max_not_smallest_gap(seeded):
    # Mirror the backfilled externalwebapp band: high, gap-pocked (no 1..661).
    _insert_item(seeded, 662, EXT_PROJECT_ID, 662)
    _insert_item(seeded, 1882, EXT_PROJECT_ID, 1882)
    seeded.commit()
    # MAX+1, not the smallest-unused 1.
    assert allocate_project_sequence(seeded, EXT_PROJECT_ID) == 1883
...
def test_allocation_is_per_project(seeded):
    ...
    seeded.commit()
    assert allocate_project_sequence(seeded, EXT_PROJECT_ID) == 663
    assert allocate_project_sequence(seeded, YOKE_PROJECT_ID) == 6
