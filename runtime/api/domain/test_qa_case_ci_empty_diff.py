...
def test_find_covering_run_omits_pull_request_event(monkeypatch) -> None:
    captured: dict[str, tuple] = {}

    def fake_github_actions(*args, **kwargs):
        captured["args"] = args
            ...
            stdout=json.dumps(
                {
                    "result": {
                        ...
                        "status": "completed",
                        "conclusion": "success",
                        "html_url": "https://github.test/actions/runs/3",
                        "head_sha": LANE_HEAD,
                    }
                }
            )
    ...
    found = empty_diff.find_covering_run(
        project="yoke",
        repo="acme/widgets",
        workflow="ci.yml",
        head_sha=LANE_HEAD,
    ...
    assert found.run_id == "3"
    assert "--event" not in captured["args"]
    assert "pull_request" not in captured["args"]
