...
def test_project_trunk_declares_why_it_has_no_item_stamp():
    assert (
        INTEGRATION_TRUNK_LADDER.recording_scope
        is SatisfierRecordingScope.RESOLUTION_ONLY
    )
    assert "project-scoped" in INTEGRATION_TRUNK_LADDER.recording_reason
    assert "item identity" in INTEGRATION_TRUNK_LADDER.recording_reason


def test_boundary_prefers_the_remote_ref_over_the_local_one():
    facts = _present(OBSERVED_REMOTE_INTEGRATION_REF, OBSERVED_LOCAL_INTEGRATION_REF)
    assert (
...
def test_merge_evidence_takes_the_ci_rung_when_ci_is_declared_and_green():
    ...
    assert resolve_ladder(DONE_MERGE_EVIDENCE_LADDER, facts).rung_id == "merged_with_ci"


def test_merge_evidence_falls_to_merged_locally_without_ci():
    facts = _present(OBSERVED_MERGE_RECORDED)
    assert resolve_ladder(DONE_MERGE_EVIDENCE_LADDER, facts).rung_id == "merged_locally"


def test_declared_ci_with_no_verdict_still_reaches_the_local_merge_rung():
    facts = _present(capability_fact("ci_workflow_file"), OBSERVED_MERGE_RECORDED)
...
def test_merge_evidence_attests_when_no_implementation_branch_exists():
    ...
    assert resolve_ladder(DONE_MERGE_EVIDENCE_LADDER, facts).rung_id == "agent_attested"
...
def test_delivery_stamps_merge_only_for_a_project_with_no_target():
    ...
    assert resolve_ladder(DELIVERY_EVIDENCE_LADDER, facts).rung_id == "merge_only"
