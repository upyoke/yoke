...
from runtime.api.domain.agents_render_workspace_anchor_test_support import (
    SESSION_REGRESSION,
    apply_workspace_claim_schema,
    run_identity_pytest,
    seed_worktree_claim_rows,
)
...
def test_byte_identity_tests_are_cwd_independent(
    ...
    for label, cwd in cwds:
        rc, output = run_identity_pytest(cwd=cwd, repo_root=repo_root)
...
def test_atomic_write_refuses_main_target_under_worktree_claim(
    ...
    with init_test_db(
        tmp_path,
        apply_schema=apply_workspace_claim_schema,
    ) as db_path:
        seed_worktree_claim_rows(
            db_path,
            repo_root=repo_root,
