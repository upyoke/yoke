...
class TestRunAdd:
    ...
    def test_optional_fields(self, db_path: str, req_id: int) -> None:
        ...
        conn = connect_test_db(db_path)
        row = conn.execute(
            "SELECT score, confidence, raw_result, duration_ms FROM qa_runs WHERE id = %s",
            (run_id,),
        ).fetchone()
        conn.close()
        assert row[0] == 0.95
    ...
    def test_artifact_path_creates_linked_artifact(
        ...
        stored = Path(handle["path"])
        assert stored != screenshot
        assert stored.name == screenshot.name
        assert stored.read_bytes() == screenshot.read_bytes()

    def test_artifact_path_none_creates_no_artifact(
        self, db_path: str, req_id: int
    ) -> None:
