...
def _seed_item(conn, *, item_id: int = 5001, spec: str = "") -> int:
    conn.execute(
        ...
        "VALUES (%s, 'item', 'issue', (SELECT current_version_id FROM workflows WHERE id='issue'), 'refined-idea', 'medium', "
    ...
    def test_pass_when_claim_covers_all_budget(self, conn):
        ...
        item_id = _seed_item(
            ...
            spec=(
                "## File Budget\n\n"
                "- `runtime/api/domain/foo.py` — does X.\n"
                "- `.yoke/docs/reference/lifecycle.md` — note.\n"
            ),
        )
        t1 = _seed_target(conn, "runtime/api/domain/foo.py")
        t2 = _seed_target(conn, ".yoke/docs/reference/lifecycle.md")
