...
def test_steering_no_longer_renders_as_a_per_project_claim_target():
    """Steering is one keycap built from the session's claims, not a target.

    A per-project label here is what produced a lock row per project
    alongside the separate document row; the sessions section filters
    steering claims out before this renderer sees them.
    """
    assert (
...
def _insert_lease(
    ...
    *,
    ...
    machine = lease_key.split(":", 1)[1]
    raw = connect_test_db(db.path)
    try:
        raw.execute(
            "INSERT INTO work_claims "
            "(id, session_id, target_kind, scope, claimed_at, released_at, "
            "release_reason) VALUES (%s, %s, 'qa_admission', %s, %s, %s, %s)",
            (
                lease_id,
                session_id,
                make_qa_admission_target(machine).scope_json(),
...
def _seed_item(db, *, item_id: int, workflow_id, status) -> None:
    raw = connect_test_db(db.path)
    try:
        raw.execute(
            'INSERT INTO items (id, workflow_id, status) VALUES (%s, %s, %s)',
...
class TestItemClaimLifecycle:
    """Each item keycap says how far along its own item is."""

    def test_every_item_claim_carries_its_workflow_and_stage(
        self, board_db: BoardDB
    ) -> None:
        _seed_item(board_db, item_id=2363, workflow_id="blitz", status="reviewing")
        _seed_item(board_db, item_id=53, workflow_id="issue", status="release")
        keycaps = build_session_keycaps(
            board_db,
            "sess-A",
            [("YOK-2363", 2363, None), ("PLAT-53", 53, None)],
            active_only=True,
        )
        assert keycaps == ["YOK-2363 blitz·reviewing", "PLAT-53 issue·release"]

    def test_an_item_with_no_workflow_still_shows_its_stage(
        self, board_db: BoardDB
    ) -> None:
        _seed_item(board_db, item_id=2363, workflow_id=None, status="implementing")
        keycaps = build_session_keycaps(
            board_db, "sess-A", [("YOK-2363", 2363, None)], active_only=True
        )
        assert keycaps == ["YOK-2363 implementing"]

    def test_a_payload_without_the_lifecycle_read_still_renders_the_ref(
        self, board_db: BoardDB
    ) -> None:
        # An older board payload carries no recording of this query; the
        # keycap loses the decoration rather than the row.
        keycaps = build_session_keycaps(
            board_db, "sess-A", [("YOK-2363", 2363, None)], active_only=True
        )
        assert keycaps == ["YOK-2363"]
...
class TestActiveSessionKeycaps:
    ...
    def test_lease_keycap_separate_from_work_claim(self, board_db: BoardDB) -> None:
        _insert_lease(board_db, lease_id=1, session_id="sess-A", lease_key="QA_HOST:mac-mini-lab")
        keycaps = build_session_keycaps(board_db, "sess-A", [("YOK-1663", 1663, None)], active_only=True)
        assert keycaps == ["YOK-1663", "\U0001f512 QA_HOST:mac-mini-lab"]
    ...
    def test_all_three_primitives_concurrent(self, board_db: BoardDB) -> None:
        ...
        _insert_lease(board_db, lease_id=2, session_id="sess-A", lease_key="QA_HOST:mac-mini-lab")
        keycaps = build_session_keycaps(board_db, "sess-A", [("YOK-1663", 1663, None)], active_only=True)
        assert keycaps == ["YOK-1663 \U0001f4c123", "\U0001f512 QA_HOST:mac-mini-lab"]
...
class TestClosedSessionKeycaps:
    ...
    def test_terminal_lease_omits_release_reason(self, board_db: BoardDB) -> None:
        _insert_lease(
            board_db, lease_id=3, session_id="sess-A", lease_key="QA_HOST:mac-mini-lab", released_at="2026-05-01T00:00:00Z", release_reason="completed"
        )
        keycaps = build_session_keycaps(board_db, "sess-A", [], active_only=False)
        assert keycaps == ["\U0001f512 QA_HOST:mac-mini-lab"]
    ...
    def test_active_filter_hides_released_path_and_lease(self, board_db: BoardDB) -> None:
        ...
        _insert_lease(
            board_db, lease_id=4, session_id="sess-A", lease_key="QA_HOST:mac-mini-lab", released_at="2026-05-01T00:00:00Z", release_reason="completed"
        )
        keycaps = build_session_keycaps(board_db, "sess-A", [], active_only=True)
        assert keycaps == []
