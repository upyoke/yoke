...
1. **Canonical CREATE TABLE block** — add the column to the table's `CREATE TABLE IF NOT EXISTS` statement in `create_core_tables()` (`packages/yoke-core/src/yoke_core/domain/schema_init_tables.py`). This is the fresh-DB-init path.
2. **Additive-schema converge step** — add an `_add_column_if_not_exists(conn, "<table>", "<new_column>", "<TYPE [DEFAULT value]>")` line to `apply_additive_schema()` (`packages/yoke-core/src/yoke_core/domain/schema_init_columns.py`), NOT `apply_legacy_data_migrations()` and NOT the `apply_idempotent_migrations()` wrapper. `apply_additive_schema()` is the strictly-additive function that `converge_core_schema` runs on every server boot, so a line added here self-propagates to every already-born universe on the next deploy/boot. The column must be self-sufficient on ADD alone — nullable, or `NOT NULL DEFAULT` (Postgres populates existing rows at ADD time). A column needing a follow-up data backfill belongs in `apply_legacy_data_migrations()`, not here. The helper guards through `schema_common` column introspection, so it is safe to re-run against existing DBs.
3. **HC-schema-drift expected schema** — extend the table's `colname/TYPE,...` section in `_EXPECTED_SCHEMA_STR` (`packages/yoke-core/src/yoke_core/engines/doctor_hc_db_project_schema_expected.py`). The doctor HC `HC-schema-drift` reads this constant.
...
5. **Relevant domain wrapper field lists** — when the touched table has a domain module that enumerates field names, add the new column to those lists. Examples:
   - Items columns: `packages/yoke-core/src/yoke_core/domain/items_constants.py` (`CANONICAL_COLUMNS`, `LIST_COLUMNS`, `LARGE_TEXT_FIELDS`, `STRUCTURED_FIELDS`, `CONTENT_FIELDS`) and `packages/yoke-core/src/yoke_core/domain/mutation_fields.py` (`SUPPORTED_UPDATE_FIELDS`).
   - Flow columns: `packages/yoke-core/src/yoke_core/domain/flow.py` / `packages/yoke-core/src/yoke_core/domain/flow_crud.py`.
   - Ephemeral env columns: `packages/yoke-core/src/yoke_core/domain/ephemeral_env.py`.
...
- [ ] `create_core_tables()` CREATE TABLE updated (`packages/yoke-core/src/yoke_core/domain/schema_init_tables.py`)
- [ ] `apply_additive_schema()` `_add_column_if_not_exists` line added — NOT the legacy-data tail or the `apply_idempotent_migrations()` wrapper (`packages/yoke-core/src/yoke_core/domain/schema_init_columns.py`); this is the boot-converge, self-propagating path so the column reaches existing universes on next deploy
- [ ] `_EXPECTED_SCHEMA_STR` table section updated (`packages/yoke-core/src/yoke_core/engines/doctor_hc_db_project_schema_expected.py`)
