...
Operator-opened Claude and Codex surfaces can hold the turn with `decision/block` and re-inject a reminder that names its outs (finish the step, release the claim, or stop deliberately); cooldown or ceiling exhaustion allows the turn and records `ChainEndDeferred` at WARN with `reason=reinjection_cap_reached`, the unfinished work, and recovery. The Yoke-launched Claude CLI, Codex CLI, and Codex app-server transports are headless commands that cannot accept another prompt after their final Stop, so their relay-continuation fact is `none`. Cursor's Stop renderer cannot block on either surface, so its general continuation fact is also `none`. Those Stops are allowed with `{}` and record durable deferred-work evidence at WARN with `reason=stop_denial_continuation_unsupported`. An active claim prevents empty-session cleanup, while the Cursor CLI's liveness-only process anchor lets relay liveness prove a terminated worker dead without treating its multiplexed host as ambient session identity.
