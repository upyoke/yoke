...
Operator-opened Claude and Codex surfaces can hold the turn with `decision/block` and re-inject a reminder that names its outs (finish the step, release the claim, or stop deliberately); cooldown or ceiling exhaustion allows the turn and records `ChainEndDeferred` at WARN with `reason=reinjection_cap_reached`, the unfinished work, and recovery. An armed Monitor waiter or an open Bash/Shell `session_tool_calls` row holds Stop independently of that reminder cooldown. Claude auto-background PostToolUse carries structured `backgroundTaskId` / `timedOutAfterMs` and must not close that row — later unrelated tool calls do not erase it. Parked sessions, a later PostToolUse with an observed exit, session-end sweep, and recorded merge-queue landings remain the documented escapes. A Yoke-launched Claude CLI worker holds the same way: being headless bounds what can reach it *between* turns, not whether a denied Stop resumes the turn it is already in, and the Claude CLI resumes it — the denied Stop returns to the same process, which continues and raises `stop_hook_active` on its next Stop, so its relay-continuation fact is `supported`. Whether the Codex CLI and app-server transports do the same is unproven, so their relay-continuation fact stays `none` until a canary on those surfaces says otherwise. Cursor's Stop renderer cannot block on either surface, so its general continuation fact is also `none`. Those Stops are allowed with `{}` and record durable deferred-work evidence at WARN with `reason=stop_denial_continuation_unsupported`. An active claim prevents empty-session cleanup, while the Cursor CLI's liveness-only process anchor lets relay liveness prove a terminated worker dead without treating its multiplexed host as ambient session identity.
