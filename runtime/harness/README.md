...
- Uses `session_stale_ttl_minutes` (default 20) for sessions with no active holdings and `session_stale_ttl_with_holdings_minutes` for session-owned strategy-document locks. A persisted active work claim protects its session with no inactivity bound, including after relay restart and process exit.
...
- Reports `skipped_between_turns` with `reason=active_work_claim` for work claim holders and `reason=between_turns` for other sessions inside their retention bound. The persisted claim is read before startup and periodic cleanup and checked again at the reclaim boundary.
