...
- Reports `skipped_between_turns` for any session whose activity is fresh *and* genuinely in flight — a running turn or a live open tool call. The janitor is never the right tool to end a session mid-turn, on any harness.
