...
Stop also runs the promised-work gate before that cleanup. When the session holds a live work claim on a non-terminal, non-wait item and the latest main-agent final text is not an operator question, the gate reads the surface's manifest-owned `stop_denial_continuation` fact. For a session durably correlated with a Yoke relay launch, it also requires `relay_stop_denial_continuation`. Relative hook entrypoints such as Claude's raw `cli` value are resolved within the executor family before those facts are read.

Operator-opened Claude and Codex surfaces can hold the turn with `decision/block` and re-inject a reminder that names its outs (finish the step, release the claim, or stop deliberately); cooldown or ceiling exhaustion allows the turn and records `ChainEndDeferred` at WARN with `reason=reinjection_cap_reached`, the unfinished work, and recovery. An armed Monitor waiter or a *live* open Bash/Shell `session_tool_calls` row holds Stop independently of that reminder cooldown. Claude auto-background PostToolUse carries structured `backgroundTaskId` on the tool response and must not close that row — `timedOutAfterMs` alone and `run_in_background` on tool input are not pending evidence, and a parsed failure or nonzero exit wins. Later unrelated tool calls (including TaskOutput under a different `tool_use_id`) do not erase the original Bash row. Claude's transcript records background completion as a user message with `origin.kind=task-notification`; that is not a captured UserPromptSubmit hook stdin, Claude settings do not subscribe `Notification`, and quoted `<task-notification>` XML in an ordinary prompt must not close a live row — so prompt/message regex settlement is not an owner. An open Bash/Shell row still holds only while `open_tool_call_is_live` agrees it is current work (start stamp vs `last_tool_call_at` within the existing 60s write skew). A still-running auto-backgrounded command followed by unrelated tools after that window is indistinguishable from residue and does not hold Stop; extending that would reintroduce eternal holds for harnesses that never close Bash, and a stored pending flag would be new machinery. Parked sessions, a later PostToolUse with observed terminal failure, the session-end orphan sweep (`outcome=interrupted`), and recorded merge-queue landings remain the documented escapes. A Yoke-launched Claude CLI worker holds the same way: being headless bounds what can reach it *between* turns, not whether a denied Stop resumes the turn it is already in, and the Claude CLI resumes it — the denied Stop returns to the same process, which continues and raises `stop_hook_active` on its next Stop, so its relay-continuation fact is `supported`. Whether the Codex CLI and app-server transports do the same is unproven, so their relay-continuation fact stays `none` until a canary on those surfaces says otherwise. Cursor's Stop renderer cannot block on either surface, so its general continuation fact is also `none`. Those Stops are allowed with `{}` and record durable deferred-work evidence at WARN with `reason=stop_denial_continuation_unsupported`. An active claim prevents empty-session cleanup, while the Cursor CLI's liveness-only process anchor lets relay liveness prove a terminated worker dead without treating its multiplexed host as ambient session identity.
...
- Uses `session_stale_ttl_minutes` (default 20) for sessions with no active holdings and `session_stale_ttl_with_holdings_minutes` (default 1440) for sessions holding a work claim, session-owned strategy-document claim, or session-owned coordination lease. One short-tier base applies on every harness.
...
- Reports `skipped_between_turns` for any session the sweep spared even though the base threshold had elapsed — by the holdings-aware TTL, or by live in-flight evidence. The bucket answers "why was this one left alone?", so a session still inside the base threshold is simply fresh and is not listed. No executor is special-cased: the janitor is never the right tool to end a session between turns, on any harness.
