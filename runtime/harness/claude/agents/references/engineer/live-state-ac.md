...
**Execution:** Apply the mutation through the proper channel: migration script for DDL; the canonical `yoke <subcommand>` agent CLI for control-plane data updates (e.g., `yoke items structured-field replace`, `yoke items section upsert`, `yoke claims path widen` — use command help and the packet roster); deployment pipeline for infrastructure. If the AC involves live DB schema or destructive DDL, the migration protocol (see `.claude/agents/references/engineer/migration-protocol.md`) still applies in full.
