...
Use the canonical entry block exactly as defined in `.claude/agents/references/_shared/ouroboros-reflection-contract.md`. Set `agent: tester` and `context:` to the PREFIX-N / epic-task identifier you reviewed. Use one of the four enum category values verbatim. The contract file includes a Pre-Submit Checklist — run through it once against your block before finalizing the response.
