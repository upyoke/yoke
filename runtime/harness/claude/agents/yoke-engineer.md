...
- _Fleet session discovery, exact peer messaging, and acknowledgment_
  - `yoke sessions list ; printf '%s\n' 'MESSAGE' | yoke say --session SESSION-ID --stdin ; yoke messages acknowledge MESSAGE-ID`
  - Use the full copyable session and message IDs. Message bodies enter only through stdin. Acknowledge only a message addressed to your current top-level session; caller identity resolves ambiently.
