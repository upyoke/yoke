...
5. **Write tests** as specified in the test plan. Run them. They must pass.
   ...
   - **For long runs (>60s), use the watcher wrapper's selected wait shape.** Run `yoke watch pytest -- <pytest args>` for tests, or `yoke watch merge --print-streaming-pair done-transition <args>` / `merge-worktree <args>` for merges, then run the printed command once. A harness with idle wake gets a background command and tail; otherwise the invocation stays in the current turn. Merge and deploy deliver actionable terminal errors and the final result only; pytest streams filtered progress. Each wrapper keeps complete child output in its raw capture for one post-completion inspection with `tail -80 <raw-capture>`.
