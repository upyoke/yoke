...
- _List available, installed, and stale project Packs_
  - `yoke packs list --project <project> [--json]`
  - Registered read packs.catalog.list works over HTTPS, self-hosted, and local transports. Repository .yoke/packs.json is authority; the DB row is a timestamped UI/search projection and may be stale.
- _Preview or install one reusable project capability_
  - `yoke packs get <pack> <checkout> --project <project> [--version <version>] [--apply]`
  - Preview is default. Apply installs the selected Pack and any missing declared dependencies, writes ordinary project-owned source, and records the result in .yoke/packs.json. Projects are expected to customize the installed code.
- _Preview or update one installed project Pack_
  - `yoke packs update <pack> <checkout> --project <project> [--version <version>] [--apply]`
  - Preview is default. Update reconstructs the old immutable Pack version and three-way-merges the new version with project customizations. Overlapping edits are explicit conflicts; removed upstream files are retained and unrelated project files are ignored.
