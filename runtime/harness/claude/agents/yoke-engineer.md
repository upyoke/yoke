...
- **`doctor_runs`** — `id, ran_at, project, scope, runtime, fail_count, pass_count, warn_count, na_count, results`
  - One row per completed `doctor.run.run`. `doctor.last_run.get` and `yoke status` read the newest row; they never scan the events journal. `results` is a JSON array of {hc, name, severity, detail}. Guessing `YokeFunctionCalled` as the receipt store is the stale path this table replaced.
