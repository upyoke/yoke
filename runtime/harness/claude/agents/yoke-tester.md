...
- _Migrate legacy Pulumi operator state_
  ...
  - Migration is dry-run-default and exact-set, persists a durable retry marker, and returns redacted metadata. Stack-config registration is admin metadata-only; fetch its body through the no-store boundary with `yoke projects pulumi-stack-config get --project <project> --stack <stack> --output <file>`; execute it with `yoke pulumi exec --project <project> --stack <stack> -- preview` (also allows refresh and safe file-form import). Local execution reads aws-admin from the machine capability store and resolves the selected service's repository-bound GitHub App authorization; Actions retains ambient OIDC credentials. Generic capability and operator-state surfaces are closed.
