...
- _Migrate legacy Pulumi operator state_
  ...
  - Registered projects.pulumi_state.migrate is dry-run-default, requires the exact source stack set, and returns redacted metadata only. Fetch one stack with `yoke projects pulumi-stack-config get --project <project> --stack <stack> --output <file>`; execute it with `yoke pulumi exec --project <project> --stack <stack> -- preview` (also allows refresh and safe file-form import). Generic capability get/full-set and stack_state merge are intentionally closed.
