...
Per-project governed contract for schema and bulk-data changes against a declared authoritative DB. Applies to any project declaring a `migration_model` capability (absence = opted out). Every ticket carries a **DB claim**; the default `{"state":"none"}` means "no governed mutation declared."
...
Block messages name BOTH axes so the operator knows which side to amend.
