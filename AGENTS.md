...
## Session Continuity
- Progress Log entries are current-state checkpoints (objective, standing decisions/holds, active work, blockers, next actions, links to durable evidence). Do not restate full results or historical snapshots; distill research into docs. After compaction, reload the current-phase skill — discarded context is gone.
