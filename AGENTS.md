...
- **Flow id != run id; item-bound delivery uses Usher/start-for-item.** `yoke-hosted-production` is a flow definition; run ids look like `run-YYYYMMDD-NNN`, and `/yoke usher YOK-N` creates runs through `runs start-for-item`, writes `deployment_run_items`, executes the pipeline, then moves members to `done`.
- **Hosted release flows are project-local; Platform owns promotion.** Yoke items use `yoke-hosted-stage`, `yoke-hosted-production`, or `yoke-hosted-production-hotfix`; the Yoke bridge allocates the annotated release tag, waits for the wheel and server-image factories, and dispatches Platform's single promotion boundary. Platform items use `platform-stage`, `platform-production`, or `platform-production-hotfix` and dispatch the Platform train directly. Do not hand-create release tags, hand-edit `yoke-release-pin.txt`, or dispatch component deploy workflows as the normal path.
...
- **A push is not release authority.** Merge the item, then let its selected
  ...
  proof, and any Production promotion. Neither Yoke nor Platform deploys from
  a branch push. The private Platform deploy runbook is the detailed operator
  reference for this installation's cross-repository release train.
