...
- **Environment redeploys are item-less by design, but prod direct DB is admin-only.** For normal item-bound prod delivery, use `/yoke usher YOK-N` and the wrapped Yoke API/HTTPS surfaces it drives. Ticketless Yoke prod/stage redeploys are operator-attended source-dev/admin or audited break-glass runs: resolve the target branch SHA from an explicit source checkout (`git -C <source-checkout> fetch origin <target-branch>` then `git -C <source-checkout> rev-parse --short=12 FETCH_HEAD`), create a zero-item run under the project that OWNS the deploy environments/flows — which may be a distinct project re-parented off the product's own project, so pass that deploy-owner project, not necessarily the product's (`YOKE_ENV=prod-db-admin python3 -m yoke_core.cli.db_router runs create-run <deploy-owner-project> yoke-prod-release --target-env prod --created-by operator`), then execute that run id through the deploy watcher, pointing `--product-src` at the target-SHA source checkout so the deploy runs the pinned product's code (the watcher executes the invoking checkout's code, so a checkout that lags the pinned product derives resource names from stale logic and strands infra): `YOKE_ENV=prod-db-admin python3 -m yoke_core.tools.watch_deploy --product-src <source-checkout> -- {run-id} --image-tag <git-short-sha>`. Do not use `YOKE_ENV=prod-db-admin` as a routine retry after a product read or domain wrapper fails.
...
- **The DB is the source of truth.** Project-local `.yoke/BOARD.md` is a generated view. Item body content is rendered on demand via `items get YOK-N body`. Never edit generated views directly.
