...
- **Capture-first: any non-trivial command is captured to a temp file.** Never pipe a live invocation into `tail`, `head`, or any truncating consumer — that discards failure context and masks the exit code. Applies wherever output matters on failure (roughly >5s): tests, merges, deploys, syncs, QA, renders, installs, builds, long git ops. Use `_tmp=$(mktemp /tmp/yoke-cmd.XXXXXX); <command> >"$_tmp" 2>&1; _rc=$?`, inspect, exit `$_rc`.
...
- **A command that outlives its yield is still running — continue it, never relaunch it.** Every harness hands a long command back before it finishes; none of those handoffs is an interruption, and a quiet watcher is not a dead one. Re-run only once the process is verifiably gone: a relaunch beside a live invocation spends the shared resource twice and can cancel work the first was about to finish. Taught exception, for an overlong *local* check only: interrupt past a minute, keep the capture, commit, continue on CI.
...
- **Run lane-source commands through `yoke dev run -- <command>`** — the installed `yoke` and a bare `python3 -m ...` resolve main's editable install, never your claimed lane. The local verification default is `yoke watch pytest --impacted main --bounded`, and it runs on CI; `--local` is only a small targeted check expected to finish in about one minute, and uncommitted work does not justify a slow local run. Both shapes inject xdist `-n auto` unless `-n` is passed. The full three-anchor sweep is CI's job and the doctrine names its anchors; the runner behind it is `python3 -m yoke_core.tools.run_tests`, and changed-path lint is `yoke dev ruff-changed --base <ref>`.
