...
- **Flow id != run id; item-bound delivery uses Usher/start-for-item.** `yoke-hosted-stage-no-ci-gate` is a flow definition; run ids look like `run-YYYYMMDD-NNN`, and `/yoke usher YOK-N` creates runs through `runs start-for-item`, writes `deployment_run_items`, executes the pipeline, then moves members to `done`.
