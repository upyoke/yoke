...
- **Capture-first: any non-trivial command is captured to a temp file.** Never pipe a live invocation into `tail`, `head`, or any truncating consumer — that discards failure context and masks the exit code. Applies wherever output matters on failure (roughly >5s): tests, merges, deploys, syncs, QA, renders, installs, builds, git. Use `_tmp=$(mktemp /tmp/yoke-cmd.XXXXXX); <command> >"$_tmp" 2>&1; _rc=$?`, inspect, exit `$_rc`.
- **Yoke's own adapters run bare.** Registry-covered commands (`yoke <subcommand>`, `db_router`, `service_client`) are short: run them bare, add `--json` for structure, never wrapped in redirection, pipes, truncators, or `| python -c` soup. **Piping any `yoke` invocation into `head` or `tail` is refused** by `lint-yoke-adapter-stderr-visibility` — a byte window drops the rest of the answer and reports the truncator's exit status. Narrow the request instead; capture to a file when you want the whole thing kept.
- **A read prints its whole answer, and names anything it did not print.** Nothing condenses by size and nothing goes quiet. Two shapes exist where a routine question and a deliberate one have very different costs: **`yoke messages list` serves one row per message** (`yoke messages get MESSAGE-ID` for a body), **`yoke qa plan get` serves cases, methods, target and verdicts** (`--full` for probe source and proof evidence), and **`yoke workflow execution-instruction resolve` serves headings** (`--full` for the prose you must read and obey — filing recipes always pass it). Each summary carries the exact command or flag that returns the rest. Separately, one field of one mutation that restates state you did not ask it to change (today only `sessions.touch`'s `session` row) prints as `<not printed: N bytes …>` named on stderr; everything else, `--json`, and refusals print in full. A receipt prints after the command ran, so `--json` is a flag on an invocation, never a reason to repeat a write.
...
- **Run lane-source commands through `yoke dev run -- <command>`** — the installed `yoke` and a bare `python3 -m ...` resolve main's editable install, never your claimed lane. The local default is `yoke watch pytest --impacted main --bounded`, which runs on CI; `--local` is only a small targeted check expected to finish in about one minute, and uncommitted work does not justify a slow one. Both shapes inject xdist `-n auto` unless `-n` is passed. The full sweep `yoke watch pytest -- runtime/api/ runtime/harness/ tests/` is CI's job; its runner is `python3 -m yoke_core.tools.run_tests` and changed-path lint is `yoke dev ruff-changed --base <ref>`.
