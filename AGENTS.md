...
A second rule inline, because violating it silently serializes this installation's concurrent stage and production pair:
- **A release here is two concurrent runs on one lineage** — stage on the HTTPS control-plane connection, production on the paired `prod-db-admin` connection, because production replaces the API serving the control plane. Drive them together; that overlap is correct here because stage exists only to test the live control plane and a Yoke stage run carries no members. Do not lift the pair onto a project whose stage is a real pre-production gate.
