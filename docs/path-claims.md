...
| Subcommand | Purpose |
|---|---|
| `yoke claims path register --item YOK-N --paths a,b,c [--allow-planned] [--tentative-paths a]` | Declare a path claim for an item. Resolves paths to canonical `path_targets` rows, runs overlap classification, transitions to `planned` (or `blocked` when a serial-via-dep upstream is named). `--integration-target` defaults to the project trunk (`projects.default_branch`, fallback `main`). With `--allow-planned`, missing paths mint as `planned`; `--tentative-paths` (subset of `--paths`, requires `--allow-planned`) mints that subset as `tentative`. |
...
To express "work item B waits for work item A", use the item-dependency mutation flow (`yoke items dependency add|list|update|remove`) rather than a path-claim-specific command. The path-claim system reads `item_dependencies`: register auto-populates `blocked_reason` so B lands in `state='blocked'` without `--upstream-claim-id`; widen and classify_overlap inherit the same posture in both directions. `--upstream-claim-id` is an advanced escape hatch for multi-claim coordination, not the default. When a claim releases, `propagate_release_unblock` re-classifies downstream blocked claims via direct `blocked_reason` references and the canonical satisfaction evaluator, including `fact:deployed:<environment-name>`, before flipping `blocked → planned`.
