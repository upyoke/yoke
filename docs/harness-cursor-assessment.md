...
| Stop discipline | empty stdout | exactly `{}`, short-circuit on `stop_hook_active` | `stop` accepts `followup_message` (auto-submits next turn, capped by `loop_limit`) — Yoke must deliberately *not* emit it |
