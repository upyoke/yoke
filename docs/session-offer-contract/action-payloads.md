...
Normal `session-end` is guarded by two checks: (1) the persisted chain checkpoint — a chainable, within-budget checkpoint whose `handler_outcome` still represents pending work makes the shared end-session surface return `CHAIN_PENDING`; terminal-item closeout changes a matching outcome to `terminal_item_closed`, which preserves live-loop re-offer while allowing an empty final hook to end — and (2) the active-claim guard — if the session holds any unreleased claims, `end_session()` returns `ACTIVE_CLAIM` and refuses to terminate. `--force` does not bypass `CHAIN_PENDING`; callers must pass the explicit chain-end override flag plus a non-empty rationale, which emits `ChainDeclineOverridden`. Claims are released through the claim lifecycle (completed, handed_off, finalize-exit, offer-override) or stale-session reclamation (`yoke sessions reclaim-stale --confirm`). A holder the fleet probe already reports as `claimed_by_stale` is freed by that sweep on the probe's short TTL, or immediately by `yoke sessions terminate SESSION-ID --reason R` (which releases the session's held work claims). For operator-only stranded claims, use the human-only `python3 -m yoke_core.api.service_client claim-release` CLI.
