...
That leverage has a hard boundary: live code and current-state docs must explain themselves without the planning artifacts that produced them. Work items, strategy docs, plans, phases, task numbers, and acceptance-criterion labels disappear into history. The repository remains. Agents must translate planning context into codebase-reader language before naming, writing, coding, documenting, or commenting.
