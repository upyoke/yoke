...
- Model identity arrives late, on one event only, and that event cannot afford the hook command. Every Cursor payload carries a `model` field, but on the terminal agent it is the literal `"default"` — the vendor's word for "whatever the user configured" — including on `sessionStart`. Only `afterAgentThought` names a concrete model (`model_id`, plus a variant-qualified `model`), and it fires inside the token stream with the stream held open across the hook: past roughly 250ms the run dies as `RetriableError: WritableIterable is closed`, and starting the Yoke CLI costs ~0.23s before any Yoke code runs. It is therefore the one hook wired to **shell only** — it spools its payload and echoes `{}` (an empty reply drops the stream too) — while the next ordinary hook drains the spool and ships the model through the existing client-identity channel. This is a duration limit, not a deny-capability one: `beforeShellExecution` and `beforeReadFile` are deny-capable and run the full command safely because they fire between operations. Full measurements in [Cursor Harness Integration Assessment](harness-cursor-assessment.md); owner `yoke_harness.hooks.cursor_model_spool`. The IDE surface names its model on `sessionStart` and needs none of this.
