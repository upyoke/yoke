...
- Model identity arrives late and on one event only. Every Cursor payload carries a `model` field, but on the terminal agent it is the literal `"default"` — the vendor's word for "whatever the user configured" — including on `sessionStart`. Only `afterAgentThought` names a concrete model (`model_id`, plus a variant-qualified `model`), so it is wired to the `AgentModelReported` verb whose handler re-registers the session and heals the placeholder. The IDE surface names its model on `sessionStart` and registers correctly without waiting for the first turn. `AgentModelReported` is Cursor-only: the Claude renderer skips it, because an unknown event key in `settings.json` costs every hook in the file.
