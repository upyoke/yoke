...
| declaration-corrected target | Stored digest moved because environment declared facts changed, same environment row and subject | `yoke qa requirement rebind-target` — see [Execution-target rebind](execution-target-declaration-rebind.md) |
