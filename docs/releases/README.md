...
1. Run the canonical test gate on the exact release commit and merge it to
   `main`.
2. Deliver the item through `yoke-hosted-stage-warm-gated` or
   `yoke-hosted-production-hotfix-warm-gated`; the earlier `-no-ci-gate` and
   `yoke-hosted-production` definitions are disabled history. The active flow's scoped GitHub App
