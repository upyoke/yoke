...
GitHub sync operations route through the Python-owned `yoke_core.domain.backlog_github_sync` helper family and service-client entrypoints. Repo and token resolution flow through the canonical project-auth helper at `yoke_core.domain.project_github_auth.resolve_project_github_auth`, which returns a typed result envelope: the project's `owner/repo` (read from `projects.github_repo`), the materialized token, and a ready-to-thread subprocess env dict for `gh` calls.

**One GitHub contract for Yoke and Buzz.** Both projects resolve through the same surface — Yoke is no longer a silent special case for "ambient default repo/auth." `projects.github_repo` carries the repo for every project; `capability_secrets[project, github, token]` carries the token for every project. GitHub token writes import the value into Yoke's `capability_secrets` store as `source=literal`; file/env-backed project secret rows are not a live storage shape for GitHub auth.
...
**Fail-closed on missing or broken auth.** When the resolver cannot produce a usable repo + token (missing capability, missing repo, missing token, invalid project secret source, invalid/unauthorized token, or GitHub REST transport failure), it raises a typed diagnostic with a concrete `repair_command_hint`. Yoke-owned callers surface the diagnostic instead of silently falling through to host credentials or treating the remote as empty. Operators repair via `yoke projects capability secret set --project <project> --cap-type github --key token <token>` or via `yoke doctor run --fix` — the diagnostic's `repair_command_hint` names the exact command. The runtime never instructs the operator to authenticate at the host CLI level as the answer to a project-auth/config problem.

**Cross-project coverage.** Item sync, status comments, issue close/reopen, body/title sync, resync repair, and doctor GitHub checks all route through the canonical resolver. Resync iterates per project; each iteration calls `resolve_project_github_auth(project)` and uses PAT-backed REST calls for that project. Doctor's GitHub orphan and wrong-repo checks validate label coverage and confirm that each item's GitHub issue exists in the correct repo for its project.

**Per-project sync switch precedes auth.** A project with `projects.github_sync_mode='backlog_only'` never reaches the resolver: every sync helper skips it with one mode-language log line (return code 0, flows continue), and resync excludes it from fetch/classification/repair. The skip is policy, not an auth failure — a backlog-only project needs no GitHub PAT. See [github-sync.md](../github-sync.md).
