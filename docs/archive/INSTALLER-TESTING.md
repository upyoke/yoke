# Installer Testing (Archived)

> Archived on 2026-07-27 after the installer campaign moved into registered
> QA plans, Machine methods, and `test-machine` capability operations. This
> file is retained as historical context only: it is not active operator
> guidance, must never be parsed as a live scenario catalog, and must not be
> used in place of the registered host-baseline and case-execution surfaces.
