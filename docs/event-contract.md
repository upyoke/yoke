...
| `session_id` | Ambient chain: `$YOKE_SESSION_ID` → `$CLAUDE_SESSION_ID` → `$CODEX_SESSION_ID` → `$CODEX_THREAD_ID` → process-anchor ancestry registry → `unknown` (`yoke_core.domain.session_ambient_identity`). A surface that resolves nothing records its declared empty/sentinel value; no surface invents an id. | Always populated |
