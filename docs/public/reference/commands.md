...
3. **Merge Execution** (skip if `--deploy-only`) --
 ...
 - **Merge engine:** Standalone items merge through `yoke merge item`. On merge-queue projects the default call opens/arms the pull request, records `in merge queue since T`, and exits; re-enter after the landing notification to record evidence and close out. `--wait` keeps the inline poll for process-safe operators and Codex/Cursor, never Claude. A red required-check set (failed required checks, nothing in flight) ends `--wait` immediately as a terminal failure, not a poll-budget timeout. Non-queue projects retain the local locked merge. Epic items pass the epic_ref argument to the merge engine directly. `--keep-remote` on `merge_worktree` suppresses remote branch deletion so ephemeral environments persist.
