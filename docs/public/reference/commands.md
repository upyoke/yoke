...
3. **Merge Execution** (skip if `--deploy-only`) --
 ...
 - **Merge engine:** Standalone items merge through `yoke merge item`. On merge-queue projects the default call opens/arms the pull request, records `in merge queue since T`, and exits; re-enter after the landing notification to record evidence and close out. A re-entry over a landing the control plane already recorded skips queue admission entirely and runs only that close-out, so a landing whose waiter died is recoverable rather than refused by an admission gate reading a train that has already run. `--wait` keeps the inline poll for process-safe operators and Codex/Cursor, never Claude; the landing is recorded from GitHub either way, because the pull request number is written to the item when it opens and the control-plane observer reads that pull request whether or not anything is waiting on it. A red required-check set (failed required checks, nothing in flight) ends `--wait` immediately as a terminal failure, not a poll-budget timeout. Non-queue projects retain the local locked merge. Epic items pass the epic_ref argument to the merge engine directly. `--keep-remote` on `merge_worktree` suppresses remote branch deletion so ephemeral environments persist.
