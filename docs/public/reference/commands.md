...
Each command is a nested skill at `.agents/skills/yoke/{name}/SKILL.md`. Harnesses expose those commands through their native skill or slash-command surfaces; the shared `SKILL.md` frontmatter is the single authored metadata source. Non-native harness surfaces invoke the same commands through their harness adapter's route wrapper (see the Harness Bootstrap Contract, yoke source-repo doc `docs/harness-bootstrap.md`, for command classification and the Hook Parity Map, yoke source-repo doc `docs/hook-parity-map.md`, for hook availability by harness). The operator-readable Atlas of the Yoke agent-facing surfaces (function ids, wrapped `yoke` subcommands, tool-shaped CLI adapters, permanent boundaries, pending rows, live contradictions) lives in the yoke source-repo doc `docs/atlas.md`; each command below resolves to one or more registered function calls.
...
3. **Merge Execution** (skip if `--deploy-only`) --
 ...
 - **Merge engine:** Standalone items merge through `yoke merge item`. On merge-queue projects the canonical `yoke watch merge --print-streaming-pair merge-item -- PREFIX-N --wait` call opens/arms the pull request and follows it to a terminal reading. The shared selector reads the caller's manifest wake capability and current control-plane reachability: a verified route gets the background subscription and may release the selector, while no route or an unknown answer blocks in-turn. A bare default call can still return `landing_pending=true`; rely on a later completion message only when the selector recorded a reachable route, otherwise re-enter through the canonical wait. A re-entry over a landing the control plane already recorded skips queue admission entirely and runs only that close-out, so a landing whose waiter died is recoverable rather than refused by an admission gate reading a train that has already run. The landing is recorded from GitHub either way, because the pull request number is written to the item when it opens and the control-plane observer reads that pull request whether or not anything is waiting on it. A red required-check set (failed required checks, nothing in flight) ends `--wait` immediately as a terminal failure, not a poll-budget timeout. Non-queue projects retain the local locked merge. Epic items pass the epic_ref argument to the merge engine directly. `--keep-remote` on `merge_worktree` suppresses remote branch deletion so ephemeral environments persist.
