...
- `yoke qa plan run --deployment-run-id RUN --plan PLAN --project P` — run-wide, and refused on a run pinning an item-scoped QA stage
...
# (or --head-sha). --raw-result is evidence text, not the run identity.
yoke qa run add \
