...
The dispatcher always emits `YokeFunctionCalled`. Repeated calls with the same `(function, request_id)` emit `DispatcherIdempotencyReplay` and return the cached response verbatim. `YokeFunctionCalled` carries compact metadata — function, target, outcome, duration, request identity, payload and result byte counts plus checksums, and bounded error details on failure — never the result document, which the caller already holds on the response. The dedup store is the `function_call_ledger` table (exact `request_id` match, written before the emission so a telemetry failure cannot skip it; rows expire after the replay TTL via the events retention prune) — events stay telemetry; the ledger owns the replay decision. Partial-state failures (the primary write succeeded but a downstream sync degraded) return HTTP 207 with `success=true`, `warnings=[...]`, and a `DispatcherDownstreamDegraded` row in `events`. See the yoke source-repo doc `docs/event-catalog.md` for the envelope schemas.
