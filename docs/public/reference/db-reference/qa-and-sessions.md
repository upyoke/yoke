...
- `session_stale_ttl_with_holdings_minutes` (default `1440`) — the minimum tier for a session holding any of those three active resources. It prevents a long foreground command from losing its claim or lock merely because no tool-boundary heartbeat landed. Ambient janitors (session-start included) keep this holdings TTL. `yoke sessions reclaim-stale --confirm` reclaims an item holder the fleet probe already reports as `claimed_by_stale` on the probe's short TTL; `yoke sessions terminate SESSION-ID --reason R` releases held work claims immediately.
