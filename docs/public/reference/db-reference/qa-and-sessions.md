...
A steering session spends its attention on whatever it is doing, and the fleet changes underneath it. The fleet report is that negative space, composed server-side for the holder of a project's steering claim: work the scheduler calls runnable that nobody picked up, work whose owner was released and never replaced, claim holders that have gone quiet, the live item claims, and which `(machine, surface)` pairs a launch could actually reach — read through the same eligibility composition the launch preview uses, so the report can never name a surface the launch plane would refuse. It reports; nothing in it staffs anything.
