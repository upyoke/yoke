...
**A lease is rendered whole or not at all.** The lease is one settlement unit — its receipts are marked `injected` together — so the renderer expands every body the lease holds and summarizes only the pending messages it never leased. Fitting is decided further out, where the reply is actually composed: on the envelope channel by the decision renderer, and on the raw-stdout channel by the delivery module itself, since stdout bypasses that renderer entirely. Either the whole block is carried and every receipt settles `injected`, or the composer replaces it with the overflow pointer, every receipt stays `pending`, and the pointer names each message with its own `yoke messages get` / `yoke messages acknowledge` line. A backlog too large for one composed block is therefore pointed at on every hook rather than partially shipped: nothing is lost, and no receipt outruns the text that carried it.
