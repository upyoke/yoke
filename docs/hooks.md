...
**Whole messages that fit are injected; the rest stay pending.** Fitting is decided where the reply is actually composed: on the envelope channel by the decision renderer, and on the raw-stdout channel by the delivery module itself, since stdout bypasses that renderer entirely. The composer admits as many whole message bodies as fit the harness cap, records `inline_overflow` only for a body that cannot fit even alone, and leaves every other leased receipt `pending` (`deferred_for_budget`) for the next hook's own budget. An overflow pointer names that unsplittable body with `yoke messages get` / `yoke messages acknowledge` — the plain get yields the body, and `--json` puts `body`, `message_id`, and `state` on `result` so a JSON reader following the pointer still sees it. Overflow is named on the attempt (`skip_reason`) and bounded to three tries; later hooks stop leasing that receipt. A hook that already attempted an envelope is not an absent operator, so overflow must not raise a desktop wake notice. No receipt is marked `injected` unless its body envelope actually shipped.
