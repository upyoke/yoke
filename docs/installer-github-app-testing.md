...
| `GITHUB-033` | `prepared-yoke` | Compare fresh local default, explicit local App profile, and a service-published product profile | Fresh local authorization uses the release-bundled Yoke by upyoke.com profile without flags, ambient metadata, or a development-App fallback; a complete local override works only for local mode; a complete published product profile drives the HTTPS flow |
