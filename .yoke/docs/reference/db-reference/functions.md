...
| `qa.run.add` / `qa.run.complete` | `"item"` | `yoke_core.domain.handlers.qa_browser_writes` — the two-phase capture shape (`add` lands started/captured rows, `complete` finalizes in place); both verify the run belongs to the targeted requirement and emit `QARunStarted`/`QARunCaptured`/`QARunCompleted` by field presence. A capture that completes with no verdict, on a case whose verdict comes from a reviewer, records `case_outcome='needs_review'` — that is how the release proof gate pairs the capture with its later review. CLI adapters: `yoke qa run add` / `yoke qa run complete`. |
