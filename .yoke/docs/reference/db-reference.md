...
- [machine-local-evidence.md](db-reference/machine-local-evidence.md) — `session_evidence_fetches` and `session_control.evidence.get`: reading one machine's own relay logs, watcher captures, and native diagnostics from a seat on any other machine, through the owning machine's relay.
- [steering-fleet-report.md](db-reference/steering-fleet-report.md) — what the steering seat cannot see from inside its own turn: available work, quiet holders, undelivered messages, unregistered launches, abandoned launches, items landed without close-out, and dead waits.
...
- [item-landings.md](db-reference/item-landings.md) — `item_landings`: every landing an item made, appended and never replaced, keyed on the merge commit. The item's own `merged_at` and `merge_queue_*` markers hold only the newest.
- [release-delivery.md](db-reference/release-delivery.md) — how a merged item's release is verified and announced: the two QA authorities release-to-done reads, the human-verdict notice, and the owner delivery notice.
...
- [functions.md](db-reference/functions.md) — Yoke function-call surface: envelope, registry, claim-verification matrix, and the function ids that own structured-field writes, epic-task amendment, lifecycle transitions, claim mutation, QA writes, and orchestration. Render the operator-readable Atlas of those surfaces locally with `python3 -m yoke_core.tools.atlas_render_docs render`.
...
| `deployment_runs` | `carried_work` | lineage derivation with resolved items, bare commits, and named empty reasons |
