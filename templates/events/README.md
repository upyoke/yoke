...
Reference implementations of the [Structured Logging Standard](../../docs/structured-logging-standard.md) for each source type. Copy the relevant template into your project and adapt.

## Files

| File | Source Type | Language | Standard Section |
|------|------------|----------|-----------------|
| `events.py` (+ `events_props.py` sibling) | backend | Python | Section D, Template 2 |
| `events.ts` (+ three `events_*.ts` siblings) | frontend | TypeScript | Section D, Template 3 + Section E |
| `api-route.ts` | frontend API | TypeScript | Section D, Template 3 (receiver) |

### Files in this template (backend Python)
...
### Files in this template (frontend TS)
...
| `PROJECT` | `yoke` | Project identifier |
...
| `NEXT_PUBLIC_PROJECT` | `yoke` | Project identifier |
