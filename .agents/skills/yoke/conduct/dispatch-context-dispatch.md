...
Extracted from `dispatch-context.md`. Covers Engineer dispatch orchestration (5g), main merge (5h), Tester dispatch reference (5i), post-PASS advancement (5o), and task-lane auto-chaining (5p).
...
**Branch-ahead detection:** Before dispatching the Engineer, check for
pre-existing implementation commits on the task worktree branch. A first task
whose HEAD is ahead of its `TASK_BASELINE` skips Engineer dispatch and goes
directly to the Tester. A later task in the same chain still dispatches the
Engineer because its baseline intentionally differs from the branch merge
base.
...
if [ "${_attempt_{_id}}" = "1" ]; then
 ...
 # Only skip for the first task on a branch (TASK_BASELINE == merge-base).
...
After a PASS verdict, update the generated task status. The item-level and
task-level status systems are distinct: the `review_insert` call that recorded
the PASS verdict auto-advances the task to `reviewed-implementation`, so no
...
After a PASS verdict on a generated task, find the next dispatchable task in the dispatch chain. The chain advances sequentially, but **skips** tasks whose dependencies are not yet met instead of pausing the entire chain:
