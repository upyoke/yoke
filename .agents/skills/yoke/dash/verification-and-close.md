...
the recorded landing identity, a fast-forward onto that merge — including
...
address. One terminal report per work leg — the turn you send it in, under the
claim you hold then — reaches the seat once, so a reworded retry in that same
turn deduplicates rather than arriving twice, and a send answering `Collapsed
into an earlier message` did not deliver the body you just sent. A completion
you are resumed to do is reported from a later turn and is delivered, whether
or not the resume hands you a fresh claim; never release an unfinished lane
merely to be heard. Ending a turn sends no
