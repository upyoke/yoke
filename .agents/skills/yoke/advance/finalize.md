...
The `claims.work.release` handler delegates to `yoke_core.domain.sessions.release_work_claim_for_execution`, which releases the claim and releases session focus atomically. The work claim is the only writer of `current_item_id`: acquisition sets it, and every release path frees it through `release_item_focus_if_current`, which re-points focus at the session's newest remaining item claim. Create and status-mutation paths (`backlog.record_touched_item`) record `recent_item_id` only, so filing or updating an item never makes a session read as working it.
