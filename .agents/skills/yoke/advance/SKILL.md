...
Items that require no code changes (validation, proof, guidance updates) should use `--no-worktree` when advancing to `implementing`. This sets `worktree = NULL`, which tells the done-transition engine (`runtime/api/engines/done_transition.py`) to skip the empty-branch guard (exit 8).
