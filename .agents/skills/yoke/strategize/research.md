...
The validator is read-only and never mutates the plan. It reads the tracked rendered view at `.yoke/strategy/MASTER-PLAN.md`, which the writer-commits-history flow keeps fresh; if `HC-strategy-render-staleness` has flagged stale views, run `yoke strategy render --target-root "$REPO_ROOT"` first so the validator sees the DB authority's current content. Any edits to `MASTER-PLAN.md` still flow through the propose and approve phases as with any other SML change.
