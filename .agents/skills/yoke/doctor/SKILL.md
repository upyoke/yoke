...
- GitHub-dependent health checks (sync-completeness-legacy, orphan/missing/comment-sync HCs) resolve credentials through `yoke_core.domain.project_github_auth.resolve_project_github_auth` and call GitHub REST/GraphQL via the PAT capability — they do NOT require the host `gh` CLI. Bidirectional sync HCs delegate detection and repair to the internal resync engine in doctor format, which also uses PAT-backed REST. The doctor engine forwards `--fix` automatically; the agent never runs a host shell-out itself.
