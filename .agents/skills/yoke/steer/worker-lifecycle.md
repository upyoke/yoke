...
it when its delivery clears, when its own item-scoped QA is accepted, or when a
QA stage needs it, and it closes out and
reports then. Any prompt clears a park, so a held worker you message for
